...
// pdf-lib's standard fonts use WinAnsi encoding, which can't render emoji or other
// characters outside its range (e.g. the water-drop 💧 in a Water Ingress label).
// Strip anything WinAnsi can't encode so PDF generation never throws. A few common
// symbols are mapped to safe equivalents; everything else outside Latin-1 is removed.
const san = (s) => {
  if (s == null) return ''
  return String(s)
    .replace(/[\u2018\u2019]/g, "'").replace(/[\u201C\u201D]/g, '"')
    .replace(/[\u2013\u2014]/g, '-').replace(/\u2026/g, '...')
    .replace(/[\u2022\u25CF\u25AA]/g, '-')
    // The water-ingress droplet, BEFORE the catch-all below removes it.
    //
    // assembleWeek labels a call-out "<droplet> Job Name". The droplet is above
    // Latin-1, so it was dropped and a leak visit printed as a bare job name -
    // indistinguishable from a project on a sheet that goes to accounts and to
    // operatives, when on screen the planner marks it clearly.
    .replace(/\u{1F4A7}\s*/gu, 'WI: ')
    // Drop any remaining character above Latin-1 (emoji, symbols, etc.).
    .replace(/[^\x00-\xFF]/g, '')
    .trim()
}
...
export async function buildWeeklyLabourPDF({ weeks, week, logoUrl }) {
  ...
  for (let wIdx = 0; wIdx < weekList.length; wIdx++) {
    ...
    y -= 16
    // colour key
    page.drawRectangle({ x: M, y: y - 9, width: 9, height: 9, color: rgb(0.086, 0.639, 0.290) })
    page.drawText('Confirmed', { x: M + 13, y: y - 8, size: 8.5, font, color: INK })
    page.drawRectangle({ x: M + 78, y: y - 9, width: 9, height: 9, color: rgb(0.149, 0.388, 0.922) })
    page.drawText('Provisional', { x: M + 91, y: y - 8, size: 8.5, font, color: INK })
    y -= 18
    ...
    for (const row of wk.rows) {
      ...
      page.drawText(san(row.name), { x: M + 6, y: y - 15, size: 9.5, font: bold, color: INK })
      if (row.company) page.drawText(san(row.company), { x: M + 6, y: y - 26, size: 7, font, color: GREY })
      wk.days.forEach((dk, i) => {
        const x = M + nameW + i * dayW
        const weekend = i >= 5
        ...
        if (cell) {
          ...
          for (const e of cell.entries) {
            const l = san((e.unnamed ? `${e.unnamed} unnamed` : e.projectName) + (e.half !== 'full' ? ` (${e.half.toUpperCase()})` : ''))
            const txt = l.length > 26 ? l.slice(0, 25) + '...' : l
            const colour = e.unnamed ? rgb(0.85, 0.34, 0.10) : (e.status === 'provisional' ? rgb(0.149, 0.388, 0.922) : rgb(0.086, 0.639, 0.290))
            page.drawText(txt, { x: x + 4, y: ty, size: 7.5, font, color: colour })
            ty -= 9
          }
        }
...
// Accounts Weekly Labour Allocation: like the weekly one, but only the selected
// installers, and each day cell is tagged "O/A" where that installer has an
// overnight allowance. Each row's overnight days come from row.overnight = { [dateISO]: true }.
export async function buildAccountsLabourPDF({ weeks, week, logoUrl }) {
  const weekList = Array.isArray(weeks) && weeks.length ? weeks : (week ? [week] : [])
  const pdf = await PDFDocument.create()
  const font = await pdf.embedFont(StandardFonts.Helvetica)
  const bold = await pdf.embedFont(StandardFonts.HelveticaBold)
  ...
  for (let wIdx = 0; wIdx < weekList.length; wIdx++) {
    ...
    if (logo) page.drawImage(logo, { x: M, y: y - 34, width: 34, height: 34 })
    page.drawText('Accounts Weekly Labour Allocation', { x: M + (logo ? 46 : 0), y: y - 22, size: 18, font: bold, color: INK })
    y -= 52
    page.drawText(`Week commencing ${parseISO(wk.weekStart).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })}`, { x: M, y, size: 11, font, color: GREY })
    y -= 16
    // key
    ...
    page.drawRectangle({ x: M + 78, y: y - 9, width: 9, height: 9, color: rgb(0.149, 0.388, 0.922) })
    page.drawText('Provisional', { x: M + 91, y: y - 8, size: 8.5, font, color: INK })
    page.drawText('O/A = Overnight Allowance', { x: M + 168, y: y - 8, size: 8.5, font: bold, color: rgb(0.55, 0.2, 0.55) })
    y -= 18
    ...
    const drawHeader = () => {
      ...
      // trailing "O/A total" column header space is folded into the last day; keep simple.
      y -= headerH
    }
    drawHeader()
    ...
    for (const row of wk.rows) {
      ...
      wk.days.forEach((dk, i) => {
        ...
        if (cell) {
          ...
          for (const e of cell.entries) {
            const l = san((e.unnamed ? `${e.unnamed} unnamed` : e.projectName) + (e.half !== 'full' ? ` (${e.half.toUpperCase()})` : ''))
            const txt = l.length > 24 ? l.slice(0, 23) + '...' : l
            const colour = e.unnamed ? rgb(0.85, 0.34, 0.10) : (e.status === 'provisional' ? rgb(0.149, 0.388, 0.922) : rgb(0.086, 0.639, 0.290))
            page.drawText(txt, { x: x + 4, y: ty, size: 7.5, font, color: colour })
            ty -= 9
          }
        }
        // Overnight allowance tag for this installer + day.
        if (on[dk]) {
          page.drawText('O/A', { x: x + dayW - 20, y: y - rowH + 4, size: 8, font: bold, color: rgb(0.55, 0.2, 0.55) })
        }
        page.drawLine({ start: { x, y: y - rowH }, end: { x, y }, thickness: 0.4, color: LINE })
      })
      page.drawLine({ start: { x: M, y: y - rowH }, end: { x: M + totalW, y: y - rowH }, thickness: 0.4, color: LINE })
    ...
    // Daily overnight totals row.
    ensure(rowH)
    page.drawText('Overnight allowances', { x: M + 6, y: y - 18, size: 9, font: bold, color: rgb(0.55, 0.2, 0.55) })
    wk.days.forEach((dk, i) => {
      const x = M + nameW + i * dayW
      const t = wk.rows.reduce((s, r) => s + ((r.overnight && r.overnight[dk]) ? 1 : 0), 0)
      page.drawText(String(t || 0), { x: x + 6, y: y - 18, size: 9, font: bold, color: t ? rgb(0.55, 0.2, 0.55) : GREY })
    })
    y -= rowH
  }
...
export async function buildOperativeWeekPDF({ row, week, logoUrl, customDays }) {
  ...
  y -= 46
  page.drawText(san(row.name), { x: 48, y, size: 13, font: bold, color: INK }); y -= 20
  ...
  if (customDays) {
    for (const d of customDays) {
      ...
      const txt = san(d.entries.map(e => e.projectName + (e.half !== 'full' ? ` (${e.half.toUpperCase()})` : '') + (e.status === 'provisional' ? ' - provisional' : '')).join(', '))
  ...
  week.days.forEach((dk, i) => {
    ...
    const txt = cell ? san(cell.entries.map(e => e.projectName + (e.half !== 'full' ? ` (${e.half.toUpperCase()})` : '')).join(', ')) : '-'
