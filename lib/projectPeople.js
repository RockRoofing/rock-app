...
export function resolveProjectPeople({ jobNo, opsProjects = [], users = [], override = {} }) {
  ...
  const pick = (ovKey, ihmKey) => (ov[ovKey] != null && ov[ovKey] !== '') ? ov[ovKey] : (d[ihmKey] || '')
  const projectAddress = pick('projectAddress', 'projectAddress')
  const orderRef = pick('orderRef', 'customerOrderRef')
  const customerCompany = (ov.customerCompany != null && ov.customerCompany !== '') ? ov.customerCompany : (d.customerCompany || '')
  const customerAddress = (ov.customerAddress != null && ov.customerAddress !== '') ? ov.customerAddress : (d.customerAddress || '')
  const retentionPct = resolveRetention(ov.retentionPct, d.retention)
...
function resolveRetention(ovVal, ihmRaw) {
  if (ovVal != null && ovVal !== '') {
