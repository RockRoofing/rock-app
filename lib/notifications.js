...
export const TASK_SETS = {
  ...
  bookkeeping: {
    ...
    monthly: ['bm1', 'bm2', 'bm3', 'bm4', 'bm13', 'bm5', 'bm6', 'bm7', 'bm8', 'bm9', 'bm10', 'bm11', 'bm12'],
    label: 'Bookkeeping',
...
function renderHtml(body, replyTo) {
  const esc = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
  // Say where a reply goes, or that it goes nowhere. People reply to these assuming
  // somebody reads it; the footer is what stops that being a silent failure.
  const foot = replyTo
    ? `Sent automatically from the Rock Roofing portal. Replies go to ${esc(replyTo)}.`
    : 'Sent automatically from the Rock Roofing portal. Replies to this address are not monitored.'
  return `<div style="font-family:system-ui,Arial,sans-serif;font-size:15px;color:#1a1a2e;line-height:1.6">${esc(body).replace(/\n/g, '<br>')}`
    + `<div style="margin-top:18px;padding-top:10px;border-top:1px solid #e5e7eb;font-size:12px;color:#888">${foot}</div>`
}
...
async function sendEmail(to, subject, body, replyTo) {
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: FROM, to, subject: subject || 'Rock Roofing notification',
      // Replies reach a person when the rule names one. Without it they go to the
      // sending subdomain, which nobody reads.
      ...(replyTo ? { reply_to: replyTo } : {}),
      html: renderHtml(body || '', replyTo),
    }),
  })
...
export async function sendNotification(n, { test = false } = {}) {
  ...
  return sendEmail(to, n.subject, n.body, n.replyTo)
}
...
export async function runNotifications({ force = false, today = new Date() } = {}) {
  ...
  for (const n of list) {
    ...
    const res = await sendEmail(to, n.subject, n.body, n.replyTo)
