...
//   checklist (fixed tick list defined by the field's `items` array; the answer is
//     stored as { '<item text>': 'Yes' | 'No' | 'N/A' }).
// ───────────────────────────────────────────────────────────────────────────
...
export const IHM_SECTIONS = [
  {
    ...
    title: 'Project Team',
    fields: [
      { id: 'estimator', label: 'Estimator', type: 'team', role: 'Estimator' },
      { id: 'contractsManager', label: 'Contracts Manager', type: 'team', role: 'Contracts Manager' },
      { id: 'operationsManager', label: 'Operations Manager', type: 'team', role: 'Operations Manager' },
      { id: 'designManager', label: 'Design Manager', type: 'team', role: 'Design Manager' },
      { id: 'quantitySurveyor', label: 'Quantity Surveyor', type: 'team', role: 'Quantity Surveyor' },
      { id: 'siteSupervisor', label: 'Site Supervisor', type: 'team', role: 'Site Supervisor' },
  ...
  },
  {
    ...
    fields: [
      { id: 'projectName', label: 'Project Name', type: 'text', required: true, help: 'Enter the project name only - do NOT include the RR Project Number (it is captured in the field below). Adding the number here makes it show twice across the app.' },
  ...
  },
  {
    ...
    fields: [
      { id: 'manufacturerContacts', label: 'Manufacturer contacts', type: 'mfrcontacts' },
    ],
  },
  {
    id: 'guarantee',
  ...
  },
  {
    ...
    fields: [
      ...
      { id: 'scopeFiles', label: 'Scope drawings / documents (PDF or image)', type: 'files' },
    ],
  },
  {
    id: 'buildup',
  ...
  },
  {
    ...
    fields: [
      ...
      {
        id: 'designHandoverDocs',
        // Pushed into an already-customised stored template on read (see
        // pages/api/templates.js). Without this flag a saved template would never pick up
        // a question added in code.
        syncFromCode: true,
        label: 'ESTIMATOR - If available, has the following information been added to file 8. Design > 1. Design Handover Docs?',
        type: 'checklist',
        items: [
          'Latest Roof Plan / Elevation Mark-Up',
          'Latest tapered drawing',
          'U-Value Calcs',
          'Wind Load Calcs',
          'Rainwater Calcs',
          'Mansafe Sketch',
          'Latest Technical Submittal including separate docs',
          'Contract Drawings',
        ],
      },
      { id: 'designHandoverDocsOther', label: 'Anything else that needs adding?', type: 'long', syncFromCode: true },
    ],
  },
  {
    id: 'commercial',
    ...
    fields: [
      ...
      // MAIN CONTRACTOR'S DISCOUNT, as a percentage and mandatory.
      //
      // It was free text called "Discount", so it could hold "2.5%", "2.5", "MCD 2.5" or
      // nothing - and nothing downstream could read it. MCD is deducted from the gross on
      // every application BEFORE retention is calculated, so it has to be a number the
      // rest of the app can use.
      //
      // 0 is a valid and common answer. Blank is not - blank means nobody asked.
      { id: 'discount', label: 'Main Contractor\u2019s Discount (MCD) %', type: 'percent', required: true,
        help: 'To two decimal places. Leave blank or enter 0 if there is no discount. MCD is deducted from the gross on every application BEFORE retention is calculated, so this figure sets the Final Account on the retention register and flows straight into Edit Project Details.' },
  ...
  {
    id: 'livetasks',
    title: 'Live Project Tasks',
    fields: [
      { id: 'liveTasks', label: 'Tasks (added to Live Project Tasks on Meeting Complete)', type: 'livetasks' },
    ],
  },
  {
    id: 'risklog',
  ...
  },
  {
    ...
    title: 'Procurement',
    fields: [
      { id: 'procurement', label: 'Procurement items (added to the Procurement Schedule on Meeting Complete)', type: 'procurement' },
    ],
  },
]
