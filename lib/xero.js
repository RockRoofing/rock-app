...
// authorised bills that still have an amount due. Used by "Bills to Pay".
export async function fetchOutstandingBills(accessToken, tenantId) {
  return fetchOutstandingInvoicesOfType(accessToken, tenantId, 'ACCPAY')
}
...
export async function fetchGeneralLedgerByAccountMonth(accessToken, tenantId, fromDateStr, codeSet = null) {
  ...
  while (true) {
    ...
    for (const j of journals) {
      ...
      if (ms < fromMs) continue                        // older than our window
      const d = new Date(ms)
      ...
      for (const line of j.JournalLines || []) {
        ...
        // NetAmount: positive = debit (expense increases), negative = credit. For an
        // overhead (expense) account, spend is a positive net amount.
        const amount = line.NetAmount || 0
        if (amount === 0) continue
    ...
    // The Journals endpoint uses the highest JournalNumber on the page as the next
    // offset. Advance to the max JournalNumber seen so far.
  ...
  return { byCodeMonth, journalCount }
}
