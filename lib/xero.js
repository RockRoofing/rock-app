...
// Returns { accounts: { [accountName]: amount }, byCode: {}, rawRows } — used as
// the Bookkeeping reconciliation benchmark (what Xero says vs what the app has).
export async function fetchProfitAndLoss(accessToken, tenantId, fromDate, toDate) {
  ...
  const accounts = {}          // accountName -> amount (period total)
  if (report) {
    for (const section of (report.Rows || [])) {
      for (const row of (section.Rows || [])) {
        ...
        // last cell is the period total
        const val = parseFloat(String(cells[cells.length - 1]?.Value || '0').replace(/,/g, '')) || 0
        if (name) accounts[name] = (accounts[name] || 0) + val
      }
    }
  }
  return { accounts, sectionTitles: (report?.Rows || []).map(s => s.Title).filter(Boolean) }
}
