...
export async function fetchBalanceSheetAccounts(accessToken, tenantId, asAtDate, periods = 0) {
  const date = asAtDate || new Date().toISOString().slice(0, 10)
  // MONTHLY COLUMNS in one call - the same shape as the Balance Sheet PDF from Xero.
  // Twelve months of real, closed positions beats reconstructing one from account names
  // and hoping the signs are right.
  const url = `${XERO_API}/api.xro/2.0/Reports/BalanceSheet?date=${date}`
    + (periods > 0 ? `&periods=${Math.min(11, periods)}&timeframe=MONTH` : '')
  let res
  try {
    res = await fetch(url, { headers: { Authorization: `Bearer ${accessToken}`, 'Xero-Tenant-Id': tenantId, Accept: 'application/json' } })
  } catch (e) {
  ...
  const report = (data.Reports || [])[0]
  const num = (v) => parseFloat(String(v == null ? '0' : v).replace(/,/g, '')) || 0
  // Column headings after the first cell - one per period, most recent first.
  const header = (report?.Rows || []).find(r => r.RowType === 'Header')
  const columns = ((header?.Cells || []).slice(1)).map(c => String(c.Value || '').trim())

  // XERO'S OWN TOTALS, taken rather than recomputed.
  //
  // Summing the rows myself is what went wrong: Xero returns liabilities as POSITIVE
  // figures under their own headings, so adding everything gives assets PLUS liabilities.
  // "Net Assets" and the section totals are already correct in the report and need no
  // interpretation.
  // EVERY summary row, whatever it is called and however deep.
  //
  // The first version only matched a handful of exact labels at two levels of nesting.
  // If Xero words it differently - "Net Assets", "Total Net Assets", or nests it inside
  // another Section - nothing matched and the page silently fell back to my own sum,
  // which is the fault this was meant to fix. Take them all and let the page choose.
  const totals = {}
  const walk = (rows, depth) => {
    if (depth > 6) return
    for (const r of (rows || [])) {
      if (Array.isArray(r.Rows) && r.Rows.length) walk(r.Rows, depth + 1)
      if (r.RowType !== 'SummaryRow' && r.RowType !== 'Row') continue
      const label = String(r.Cells?.[0]?.Value || '').trim()
      if (!label) continue
      const vals = (r.Cells || []).slice(1).map(c => parseFloat(String(c.Value == null ? '0' : c.Value).replace(/,/g, '')) || 0)
      if (r.RowType === 'SummaryRow' || /^(net assets|total)/i.test(label)) totals[label] = vals
    }
  }
  walk(report?.Rows || [], 0)
  ...
  for (const section of (report?.Rows || [])) {
    ...
    for (const row of (section.Rows || [])) {
      ...
      accounts.push({
        ...
        // Most recent column, as before...
        balance: num(cells[1]?.Value),
        // ...and EVERY month, so the forecast can re-base its component lines onto the
        // last real month end rather than only its bottom line. Keeping just one column
        // was why the actual months tied and the forecast months did not.
        values: cells.slice(1).map(c => num(c?.Value)),
      })
    }
  }
  return { accounts, columns, totals, ok: true, error: null, asAt: date }
}
