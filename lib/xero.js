...
export async function fetchBalanceSheetAccounts(accessToken, tenantId, asAtDate, periods = 0) {
  ...
  const walk = (rows) => {
    for (const r of (rows || [])) {
      if (r.RowType === 'Section') { walk(r.Rows); continue }
      ...
      if (/^(net assets|total assets|total liabilities|total current assets|total fixed assets|total capital and reserves|total creditors)/i.test(label)) {
        totals[label] = vals
      }
    }
  }
