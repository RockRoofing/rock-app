...
export async function fetchOutstandingBills(accessToken, tenantId) {
  ...
  let overpayments = []
  try {
    overpayments = await fetchOutstandingOverpayments(accessToken, tenantId)
  } catch (e) {
...
// balance sits with them. Type SPEND is money out (supplier); RECEIVE is a customer
// overpayment and does not belong on the bills page.
async function fetchOutstandingOverpayments(accessToken, tenantId) {
  const out = []
  let page = 1
  const where = encodeURIComponent('Status=="AUTHORISED"')
  while (page <= 50) {
    const url = `${XERO_API}/api.xro/2.0/Overpayments?where=${where}&page=${page}&pageSize=100`
    const res = await fetch(url, { headers: { Authorization: `Bearer ${accessToken}`, 'Xero-Tenant-Id': tenantId, Accept: 'application/json' } })
    if (!res.ok) { const t = await res.text().catch(() => ''); throw new Error(`Overpayments failed: ${res.status} ${t.slice(0, 160)}`) }
    const data = await res.json()
    const list = data.Overpayments || []
    if (!list.length) break
    for (const op of list) {
      if (String(op.Type || '').toUpperCase() !== 'SPEND') continue   // supplier side only
      const remaining = parseFloat(op.RemainingCredit != null ? op.RemainingCredit : (op.Total || 0))
      if (!(remaining > 0)) continue                                  // only unallocated
