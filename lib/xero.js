...
export async function fetchPurchaseOrders(accessToken, tenantId, { status = null } = {}) {
  const parseXeroDate = (s) => {
    if (!s) return null
    // Xero MS-JSON format: /Date(1533772800000+0000)/
    const m = /\/Date\((\d+)/.exec(String(s))
    if (m) return new Date(Number(m[1])).toISOString().slice(0, 10)
    // Plain ISO / date string: 2026-07-10T00:00:00 or 2026-07-10
    const d = new Date(s)
    return isNaN(d) ? null : d.toISOString().slice(0, 10)
  }
  let url = `${XERO_API}/api.xro/2.0/PurchaseOrders?order=${encodeURIComponent('UpdatedDateUTC DESC')}`
  ...
  const data = await res.json()
  return (data.PurchaseOrders || []).map(p => ({
    ...
    // project tracking, if present (auto-fill); else empty -> manual assign
    tracking: (() => {
    ...
    // DESCRIPTIONS + QUANTITIES ONLY — never any cost fields
    lineItems: (p.LineItems || []).map(li => ({ description: li.Description || '', quantity: li.Quantity ?? null })),
  }))
}
