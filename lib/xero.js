...
// Full Chart of Accounts: every account with its code, name, type, class and status.
// Used by the Account Categorisation "Sync chart of accounts" button.
export async function fetchChartOfAccounts(accessToken, tenantId) {
  ...
  return (data.Accounts || [])
    .filter(a => a.Code)   // only accounts that have a code
    .map(a => ({
      code: String(a.Code),
