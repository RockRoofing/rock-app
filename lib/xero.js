...
// Fetch the VAT return / tax report from Xero. The exact report id varies by org and
// MTD status, so we try the common ones and return whatever comes back, plus a diag so
// the page can show the shape and we can map fields precisely.
export async function fetchVatReturns(accessToken, tenantId, fromDateStr, toDateStr) {
  const H = { Authorization: `Bearer ${accessToken}`, 'Xero-Tenant-Id': tenantId, Accept: 'application/json' }
  const diag = { tried: [], lastError: null }
  // Candidate report endpoints, most-specific first.
  const candidates = []
  if (fromDateStr && toDateStr) {
    candidates.push(`${XERO_API}/api.xro/2.0/Reports/TaxSummary?fromDate=${fromDateStr}&toDate=${toDateStr}`)
  }
  candidates.push(`${XERO_API}/api.xro/2.0/Reports/TaxReturns`)
  for (const url of candidates) {
    try {
      const res = await fetchWithRetry(url, { headers: H })
      diag.tried.push({ url: url.split('/api.xro/2.0/')[1], status: res.status })
      if (!res.ok) { const t = await res.text().catch(() => ''); diag.lastError = `HTTP ${res.status}: ${t.slice(0, 160)}`; continue }
      const data = await res.json()
      return { report: data.Reports ? data.Reports[0] : data, diag }
    } catch (e) { diag.lastError = e.message }
  }
  return { report: null, diag }
}
