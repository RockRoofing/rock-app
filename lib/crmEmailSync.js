...
function buildIndex(deals) {
  ...
  return { byTitle, byEmail, byDomain, ids }
}
  ...
  for (const mb of mailboxes) {
    ...
    for (const msg of msgs) {
      ...
      if (hit) {
        ...
        toQueue.push(msg); q++                  // deliberate BCC - always reviewable
      } else if (subjectOnly) {
    ...
    result.mailboxes.push({ mailbox: mb.email, fetched: msgs.length, matched: m, queued: q, discarded: disc, subjectOnly, isBcc, backfill: isBackfill })
