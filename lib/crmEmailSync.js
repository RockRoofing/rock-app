...
export function closeMatch(subject, index) {
  ...
  for (const t of index.byTitle) {
    ...
    // A match has to rest on real words, not on "one" or "5". Two distinctive words, or a
    // single long one that belongs to exactly one project ("ellington", "knowsley").
    const strong = matched.filter(isStrong)
    const soloOk = strong.length === 1 && strong[0].length >= 7 && (index.docFreq.get(strong[0]) || 99) === 1
...
export async function runEmailSync({ backfillMonths = 0, dryRun = false, max = 2000, detail = false, since: sinceOverride = '', mailbox: onlyMailbox = '' } = {}) {
  ...
  const result = { ok: true, mailboxes: [], matched: 0, unallocated: 0, discarded: 0, skipped: 0, junk: 0, dryRun, threshold: MATCH_THRESHOLD, margin: MATCH_MARGIN }
