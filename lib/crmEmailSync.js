...
// Junk that is junk WHATEVER thread it is on. Spam and read receipts tell you nothing
// about a project even when we know which project they belong to - a spam message can
// land in a real conversation, and filing it because the thread is known would be worse
// than dropping it.
const NEVER_FILE_SUBJECT = /^\s*(\[spam\]|\[postmaster\]|read:)/i
...
function buildIndex(deals) {
  ...
  const emailClosed = new Map()   // contact -> is the deal holding it closed

  for (const d of deals) {
    // CLOSED PROJECTS ARE INDEXED, BUT FLAGGED.
    //
    // They used to be skipped entirely, so a won job could never be suggested - and a won
    // job is exactly when the invoice chasing, the variations and the site queries start.
    // Every one of those emails landed in the review queue with no suggestion at all.
    //
    // They are still not allowed to FILE THEMSELVES on a title match. That was the
    // original reasoning and it holds: a closed project must not silently capture mail on
    // a name it shares with a live one. So it is indexed, offered as a suggestion, and
    // waits for somebody to press Accept.
    ...
    if (title.length >= MIN_TITLE_LEN) {
      ...
      byTitle.push({ closed, title, raw: d.title || '', tokens: toks, id: d.id })
    ...
    for (const key of ['contact_email', 'org_email']) {
      ...
      // FIRST WRITER WON before, which was safe while only open deals were indexed. Now
      // that closed ones are in, a finished job could claim a contact ahead of the live
      // job the same person is working on - purely on array order. An OPEN deal takes the
      // contact from a closed one; between two of the same status, first still wins.
      const held = byEmail.get(e)
      if (held == null) { byEmail.set(e, d.id); emailClosed.set(e, closed) }
      else if (!closed && emailClosed.get(e)) { byEmail.set(e, d.id); emailClosed.set(e, false) }
      const dom = e.split('@')[1]
  ...
  // ids is what a match is validated against. Open deals only meant a deliberate
  // [CRM-972] reference to a job we had already WON was rejected, and an email on a
  // filed thread stopped landing the moment the deal closed - exactly when the invoice
  // chasing starts.
  //
  // Every deal now, whatever its status. The TITLE matching above is still open-only
  // (see byTitle) so a closed project cannot start silently capturing mail on a name it
  // shares with a live one - a deliberate reference or a known thread is a person saying
  // where it goes, and that is a different thing from a guess.
  const ids = new Set(deals.map((d) => Number(d.id)))
...
export function matchEmail(msg, index, { subjectOnly }) {
  ...
  // An exact title match on an OPEN project files itself. On a closed one it is only ever
  // offered, because a finished job must not quietly capture mail on a name it shares
  // with a live one - which is the whole reason closed projects were excluded before.
  let closedTitleHit = null
  for (const t of index.byTitle) {
    if (!subject.includes(t.title)) continue
    if (!t.closed) return { dealId: t.id, reason: 'project name in subject' }
    if (!closedTitleHit) closedTitleHit = { dealId: t.id, title: t.raw, score: 1, runnerUpScore: 0 }
  }
  ...
  // Nothing filed it, but a near miss is still worth putting in front of you. A closed
  // project whose name is in the subject outranks a fuzzy score - it is an exact hit,
  // just on a job that is no longer live.
  if (closedTitleHit) return { suggest: closedTitleHit }
...
export async function runEmailSync({ backfillMonths = 0, dryRun = false, max = 2000, detail = false, since: sinceOverride = '', until = '', mailbox: onlyMailbox = '', queue = true } = {}) {
  ...
  for (const mb of mailboxes) {
    ...
    let m = 0, q = 0, disc = 0, skip = 0, junk = 0, autoReplies = 0
    for (const msg of msgs) {
      ...
      // Anything filed earlier in this same run counts too. Moved ABOVE the junk check,
      // because whether we already know the conversation now decides what happens to an
      // automatic reply.
      ...
      // JUNK - bounce-backs, out-of-office, spam-tagged mail, read receipts, calendar
      // responses. On a real month these were the bulk of everything landing in the
      // review queue, so they are still filtered.
      //
      // BUT: one on a conversation we have ALREADY FILED is kept and logged against that
      // project. "Danny is on annual leave until 01.09.26" on a live tender is worth
      // seeing when you are chasing it - the reason it was dropped was that it tells you
      // nothing about a project, which is only true when we do not know which project it
      // belongs to.
      //
      // Junk on an UNKNOWN thread is still discarded outright. It never reaches the
      // review queue, so the queue stays usable.
      if (isJunkSubject(msg.subject)) {
        if (NEVER_FILE_SUBJECT.test(String(msg.subject || ''))) { junk++; continue }
        const knownThread = msg.conversationId && index.byThread && index.byThread.has(msg.conversationId)
        if (!knownThread) { junk++; continue }
        const dealId = index.byThread.get(msg.conversationId)
        if (!index.ids.has(Number(dealId))) { junk++; continue }
        // Filed through the same batch as everything else, so it is written once per
        // deal rather than once per message.
        if (!toFile.has(dealId)) toFile.set(dealId, [])
        toFile.get(dealId).push({ ...msg, matchedBy: 'automatic reply on a filed thread' })
        m++
        autoReplies++
        continue
      }
    ...
    result.mailboxes.push({ mailbox: mb.email, fetched: msgs.length, matched: m, queued: q, discarded: disc, skipped: skip, junk, autoReplies, subjectOnly, isBcc, backfill: isBackfill })
    result.matched += m; result.unallocated += q; result.discarded += disc; result.skipped += skip; result.junk += junk; result.autoReplies = (result.autoReplies || 0) + autoReplies
...
// BULK. Deliberately one pass over the queue and one write, not a loop of single calls -
// clearing 300 rows one request at a time would be 300 round trips and 300 rewrites of
// the same key, and a failure halfway would leave the queue in a state nobody could
// reason about.
export async function dismissEmails(messageIds) {
  const ids = [...new Set((messageIds || []).map(String))].filter(Boolean)
  if (!ids.length) return { ok: true, count: 0 }
  const idSet = new Set(ids)
  const queue = (await get(UNALLOCATED_KEY)) || []
  await set(UNALLOCATED_KEY, queue.filter((e) => !idSet.has(String(e.id))))

  const list = (await get(NEVER_KEY)) || []
  const fresh = ids.filter((id) => !list.includes(id))
  await set(NEVER_KEY, [...fresh, ...list].slice(0, NEVER_MAX))
  return { ok: true, count: ids.length }
}

// File several emails against one project at once.
export async function allocateEmails(messageIds, dealId) {
  const ids = [...new Set((messageIds || []).map(String))].filter(Boolean)
  if (!ids.length) return { ok: true, count: 0 }
  const idSet = new Set(ids)
  const queue = (await get(UNALLOCATED_KEY)) || []
  const picked = queue.filter((e) => idSet.has(String(e.id)))
  if (!picked.length) return { ok: false, error: 'Those emails are no longer in the queue' }

  const existing = (await get(EMAILS_KEY(dealId))) || []
  const have = new Set(existing.map((e) => e.id))
  const merged = [...existing, ...picked.filter((e) => !have.has(e.id)).map((e) => ({ ...e, matchedBy: 'allocated by hand' }))]
  merged.sort((a, b) => String(b.date).localeCompare(String(a.date)))
  await set(EMAILS_KEY(dealId), merged)
  await set(UNALLOCATED_KEY, queue.filter((e) => !idSet.has(String(e.id))))

  // Each conversation now points at this project, so the replies follow.
  for (const conv of new Set(picked.map((e) => e.conversationId).filter(Boolean))) {
    await rememberThread(conv, dealId)
  }
  return { ok: true, count: picked.length }
}

// Move an email from one project to another. The thread link follows it, so the rest of
// the conversation lands on the right project from now on rather than continuing to file
// against the one you have just corrected.
export async function moveEmail(fromDealId, toDealId, messageId) {
  const from = (await get(EMAILS_KEY(fromDealId))) || []
  const msg = from.find((e) => e.id === messageId)
  if (!msg) return { ok: false, error: 'That email is no longer on this project' }

  await set(EMAILS_KEY(fromDealId), from.filter((e) => e.id !== messageId))

  const to = (await get(EMAILS_KEY(toDealId))) || []
  if (!to.some((e) => e.id === messageId)) {
    to.push({ ...msg, matchedBy: 'moved by hand' })
    to.sort((a, b) => String(b.date).localeCompare(String(a.date)))
    await set(EMAILS_KEY(toDealId), to)
  }

  // Point the conversation at the new project. Deliberately NOT marked do-not-assign:
  // this email belongs on a project, just a different one.
  if (msg.conversationId) await rememberThread(msg.conversationId, toDealId)
  return { ok: true }
}
