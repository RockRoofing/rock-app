...
export async function buildPreStartPDF({ project, data, logoUrl, proof, sections }) {
  const SECTIONS = Array.isArray(sections) && sections.length ? sections : PRESTART_SECTIONS
  ...
  // Move below whichever is lower: the title baseline or the logo, so the
  // project name never overlaps the logo.
  y = Math.min(logoBottom, headerTop - 34) - 14
  ...
  drawText(`Status: ${data?.stage === 'sent' ? 'COMPLETE' : 'DRAFT'}`, { color: GREY, size: 9.5 })
  gap(4); rule()
  ...
  for (const sec of SECTIONS) {
    ...
    for (const f of sec.fields) {
      ...
      } else if (f.type === 'portalaccess') {
        const rows = Array.isArray(v) ? v : []
        if (rows.length) {
          drawText(f.label, { f: bold, size: 9.5 })
          for (const r of rows) drawText('- ' + [[r.firstName, r.lastName].filter(Boolean).join(' '), r.company, r.email, r.phone].filter(Boolean).join(' | '), { size: 9.5, indent: 8 })
          gap(2)
        }
      } else if (f.type === 'meetingactions') {
        const rows = Array.isArray(v) ? v : []
        if (rows.length) {
          drawText(f.label, { f: bold, size: 9.5 })
          for (const r of rows) {
            drawText('- ' + (r.task || '(no task)'), { size: 9.5, indent: 8, f: bold })
            const meta = [r.dueDate ? `Required by: ${r.dueDate}` : '', r.responsibleLabel ? `Responsible: ${r.responsibleLabel}` : ''].filter(Boolean).join('   ')
            if (meta) drawText(meta, { size: 9, indent: 14, color: GREY })
          }
          gap(2)
        }
  ...
  const dismissed = Array.isArray(data?.dismissedIhmDocs) ? data.dismissedIhmDocs : []
  const ihmFiles = (Array.isArray(project?.scopeFiles) ? project.scopeFiles : []).filter(f => !dismissed.includes(f.url))
  ...
  if (proof) {
    ...
    line('above recipients at the time stated.', font, 8.5, GREY)
  }
