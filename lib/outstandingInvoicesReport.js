...
export async function maybeSendScheduledReport({ baseUrl, force } = {}) {
  ...
  // Evaluate against UK local time.
  const now = new Date(new Date().toLocaleString('en-GB', { timeZone: 'Europe/London' }))
  const dateStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`
  if (!force) {
    if (now.getDay() !== sched.dayOfWeek) return { skipped: `not day ${sched.dayOfWeek}` }
    if (now.getHours() !== sched.hour) return { skipped: `not hour ${sched.hour}` }
