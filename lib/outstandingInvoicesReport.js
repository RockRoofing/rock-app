...
export async function maybeSendScheduledReport({ baseUrl, force } = {}) {
  ...
  if (!force) {
    if (ukDay !== sched.dayOfWeek) return { skipped: `not day (uk day ${ukDay}, want ${sched.dayOfWeek})` }
    // Fire at or after the target hour (so a late cron tick still catches the day), then
    // the once-per-day guard stops repeats.
    if (ukHour < sched.hour) return { skipped: `too early (uk hour ${ukHour}, want ${sched.hour})` }
    if (sched.lastSentDate === dateStr) return { skipped: 'already sent today' }
  }
