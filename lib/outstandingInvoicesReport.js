import { fromEmail } from './tenantSettings'
...
const SCHEDULE_KEY = 'invoice:weekly-schedule'       // { dayOfWeek:0-6, hour:0-23, lastSentDate:'YYYY-MM-DD' }
const META_KEY = 'invoice:meta'

// Weekly report schedule. Defaults to Thursday (4) at 08:00 to match old behaviour.
export async function getWeeklySchedule() {
  const redis = await getClient()
  const s = redis ? (await redis.get(SCHEDULE_KEY)) : null
  return { dayOfWeek: s?.dayOfWeek ?? 4, hour: s?.hour ?? 8, lastSentDate: s?.lastSentDate || '' }
}
export async function setWeeklySchedule({ dayOfWeek, hour }) {
  const redis = await getClient()
  const cur = await getWeeklySchedule()
  const next = {
    dayOfWeek: Math.max(0, Math.min(6, parseInt(dayOfWeek))),
    hour: Math.max(0, Math.min(23, parseInt(hour))),
    lastSentDate: cur.lastSentDate || '',
  }
  if (redis) await redis.set(SCHEDULE_KEY, next)
  return next
}
async function markScheduleSent(dateStr) {
  const redis = await getClient()
  const cur = await getWeeklySchedule()
  if (redis) await redis.set(SCHEDULE_KEY, { ...cur, lastSentDate: dateStr })
}

// Called by the hourly cron. Sends the report iff the configured day + hour match
// "now" (UK time) and it hasn't already been sent today. force bypasses the gate.
export async function maybeSendScheduledReport({ baseUrl, force, dryRun } = {}) {
  const sched = await getWeeklySchedule()
  // Reliable UK local day/hour via Intl parts (robust across BST/GMT; the cron ticks
  // hourly in UTC, so we must evaluate the gate in UK time).
  ...
  const ukDay = wdMap[get('weekday')]
  let ukHour = parseInt(get('hour'), 10); if (ukHour === 24) ukHour = 0
  const dateStr = `${get('year')}-${get('month')}-${get('day')}`

  const diag = {
    nowUkDay: ukDay, nowUkDayName: DAYS[ukDay], nowUkHour: ukHour, nowUkDate: dateStr,
    wantDay: sched.dayOfWeek, wantDayName: DAYS[sched.dayOfWeek], wantHour: sched.hour,
    lastSentDate: sched.lastSentDate || '(never)',
  }

  if (!force) {
    if (ukDay !== sched.dayOfWeek) { const r = { skipped: `Not the scheduled day - today is ${DAYS[ukDay]} (UK), schedule is ${DAYS[sched.dayOfWeek]}`, diag }; return r }
    if (ukHour < sched.hour) { const r = { skipped: `Too early - it's ${ukHour}:00 UK, scheduled for ${sched.hour}:00`, diag }; return r }
    if (sched.lastSentDate === dateStr) { const r = { skipped: `Already sent today (${dateStr})`, diag }; return r }
  }
  if (dryRun) return { wouldSend: true, diag }
  const result = await sendWeeklyOverdueReport({ baseUrl })
  if (result && result.ok) await markScheduleSent(dateStr)
  return { ...result, diag }
}
...
export async function sendWeeklyOverdueReport({ baseUrl, extraRecipients, dryRun } = {}) {
  ...
  // The managed recipient list (portal users you add + manual emails) is the
  // definitive list. If it's empty, fall back to internal auto-recipients so the
  // report is never sent to nobody.
  let to = [...new Set((external || []).filter(Boolean))]
  if (!to.length) {
    const internal = await internalRecipients()
    to = [...new Set(internal.filter(Boolean))]
  }

  // Clear, self-diagnosing failures rather than a silent non-send.
  if (!RESEND_KEY) return { ok: false, reason: 'RESEND_API_KEY is not set', sentTo: to, from: FROM }
  if (!to.length) return { ok: false, reason: 'No recipients: add emails to the weekly recipient list, or give a post-contract/management/admin portal user an email address', sentTo: [], from: FROM }
  ...
  let errorText = ''
  if (!resp.ok) { try { errorText = JSON.stringify(await resp.json()) } catch { try { errorText = await resp.text() } catch {} } }
  return { ok: resp.ok, sentTo: to, invoiceCount: invoices.length, status: resp.status, from: FROM, error: errorText || undefined }
}
