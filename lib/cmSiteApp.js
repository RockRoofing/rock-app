...
// projectNos. Projects are matched on projectNo.
export function allowedProjects(user, projects) {
  const pa = user?.projectAccess
  if (pa == null || pa === 'all') return projects || []
  const set = new Set((Array.isArray(pa) ? pa : []).map(String))
  return (projects || []).filter(p => set.has(String(p.projectNo)) || set.has(String(p.jobNo)) || set.has(String(p.id)))
}
...
export function canAccessProject(user, projectNo) {
  ...
  return (Array.isArray(pa) ? pa : []).map(String).includes(String(projectNo))
}
