...
//
// registryConfigured() - the control database CREDENTIALS exist, so the registry
//   can be read and written. Setting these alone changes nothing about how
//   requests are served.
//
// tenancyEnabled() - the SWITCH. Only when this is on does withTenant start
//   resolving customers and refusing unknown addresses.
//
// They are separate so a customer can be registered and VERIFIED before anything
// starts depending on it. With one flag, adding the credentials would turn
// resolution on at the next deploy while the registry was still empty, and every
// request would 404 until the registration call landed. That is a live outage
...
export function tenancyEnabled() {
  return registryConfigured() && String(process.env.TENANCY_ENABLED || '') === '1'
}

// THE PLATFORM.
//
// James's own administration of every customer lives on its own address -
// admin.constructionliberation.com or similar - not on any customer's. That
// address belongs to NO customer, so tenant resolution must not 404 it, and a
// session issued there must never be usable against a customer's site.
//
// Hence a reserved id rather than null. A session carrying '~platform' is
// clearly a platform session; a session carrying no tenant at all is an old
// one from before this existed, and is rejected.
export const PLATFORM_TENANT = '~platform'

// Comma-separated in PLATFORM_HOSTS. Until it is set, there is no platform host
// and nothing changes.
export function platformHosts() {
  return String(process.env.PLATFORM_HOSTS || '')
    .split(',').map(h => normHost(h)).filter(Boolean)
}

export function isPlatformHost(host) {
  const h = normHost(host)
  return !!h && platformHosts().includes(h)
}
