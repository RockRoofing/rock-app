...
export async function buildIssuePDF({ issue, project, logoUrl }) {
  ...
  if (issue.requiredDate) {
    const [ry, rm, rd] = String(issue.requiredDate).split('-')
    drawText(`Required resolution date: ${rd ? `${rd}/${rm}/${ry}` : issue.requiredDate}`, { color: GREY, size: 9.5 })
  }
  gap(8)

  // Status box — Open by default; Resolved once a resolved date is entered.
  const resolved = !!issue.resolvedDate
  const boxW = 150, boxH = 26
  const boxColor = resolved ? rgb(0.06, 0.65, 0.35) : rgb(0.79, 0.54, 0.02)
  const bg = resolved ? rgb(0.86, 0.99, 0.91) : rgb(1, 0.98, 0.92)
  ensure(boxH + 6)
  page.drawRectangle({ x: M, y: y - boxH, width: boxW, height: boxH, color: bg, borderColor: boxColor, borderWidth: 1.2 })
  const statusText = resolved ? 'STATUS: RESOLVED' : 'STATUS: OPEN'
  page.drawText(statusText, { x: M + 10, y: y - boxH + 9, size: 10, font: bold, color: boxColor })
  y -= boxH + 6
  if (resolved) {
    const [yy, mm, dd] = String(issue.resolvedDate).split('-')
    drawText(`Resolved: ${dd ? `${dd}/${mm}/${yy}` : issue.resolvedDate}`, { color: GREY, size: 9 })
  }
  gap(10)
