...
import { createScopedCache } from './scopedCache'

const TOKEN_URL = (tenant) => `https://login.microsoftonline.com/${tenant}/oauth2/v2.0/token`
const GRAPH = 'https://graph.microsoft.com/v1.0'

// One token slot per customer. See lib/scopedCache.js - a token cached at
// module scope outlives the request and would be handed to the next one,
// whoever it belonged to.
const tokenCache = createScopedCache()
...
export async function getGraphToken() {
  ...
  return tokenCache.set({ value: d.access_token, expires: Date.now() + (d.expires_in || 3600) * 1000 }).value
}
...
// THE FULL BODY OF ONE MESSAGE, fetched when somebody actually asks for it.
//
// The sync stores only bodyPreview - Graph's own short extract - because keeping every
// full body would multiply the size of crm:emails:<dealId> many times over for text
// almost nobody reads. But "more" could then only ever expand to that extract, which is
// not what "more" means.
//
// So the body is fetched on demand, one message at a time.
//
// Requested as PLAIN TEXT via the Prefer header rather than HTML. An email body is
// untrusted input from outside the business; rendering its HTML would mean sanitising it
// properly, and plain text reads perfectly well in a card.
export async function fetchMessageBody({ mailbox, messageId }) {
  const token = await getGraphToken()
  const url = `${GRAPH}/users/${encodeURIComponent(mailbox)}/messages/${encodeURIComponent(messageId)}`
    + `?$select=body,bodyPreview,subject,receivedDateTime,hasAttachments`
