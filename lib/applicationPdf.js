...
export async function buildApplicationPDF({ app, appNumber, project = {}, prevGross = 0, prevReleases = null, trackerVariations = [], logoUrl }) {
  ...
  heading('Summary')
  {
    ...
    if (sum.release1Value > 0) row('1st Release Retention', '', money(sum.release1Value), money(sum.halfRetention))
    if (sum.release2Value > 0) row('2nd Release Retention', '', money(sum.release2Value), money(sum.halfRetention))
    y -= 4
