...
import { describeApplication, computeApplicationSummary, worksValueToDate, isMeasurableWorks, variationValue, variationValueToDate, materialLineTotal, materialValueToDate, isInstructed } from './applications'
...
export async function buildApplicationPDF({ app, appNumber, project = {}, prevGross = 0, prevReleases = null, trackerVariations = [], logoUrl }) {
  ...
  // FINAL ACCOUNT is the same document with a different title. It keeps its application
  // number and date - it IS an application for payment, and the last one - so nothing
  // about the numbering or the certificate changes.
  const desc = describeApplication(app, { prevReleases })
  const isFinal = desc.isFinal
  // The final-account title is 58 characters; at 20pt it runs off the page. Scaled to fit
  // the width available rather than truncated - the whole point of the longer wording is
  // that it is read.
  const titleX = M + (logoImg ? 60 : 0)
  const titleAvail = width - (logoImg ? 60 : 0)
  let titleSize = 20
  while (titleSize > 11 && bold.widthOfTextAtSize(san(desc.title), titleSize) > titleAvail) titleSize -= 0.5
  page.drawText(san(desc.title), { x: titleX, y: y - 22, size: titleSize, font: bold, color: INK })
  // A second line spelling out what this document is. Said twice on purpose: a PDF titled
  // Final Account that still says "Application no. 14" invites the question, and a
  // retention release is a claim the customer has to recognise before they approve it.
  const banner = [
    // Matches the title. "This is the last application" asserts finality, which is the
    // one thing the new wording is careful NOT to do - the final account is proposed
    // until it is agreed, and the payment claimed on it is still interim.
    isFinal ? 'Our proposed final account for this project. Payment claimed is interim and remains subject to agreement.' : null,
    desc.releases.length ? `Includes the ${desc.releases.join(' and the ').toLowerCase()}` : null,
  ].filter(Boolean).join('   -   ')
  if (banner) {
    page.drawText(san(banner), { x: M + (logoImg ? 60 : 0), y: y - 36, size: 9, font: bold, color: INK })
  }
  // The banner is drawn 6pt below where the cursor would normally land, so the extra
  // line needs the extra space or the project name prints on top of it.
  y -= banner ? 44 : 30
  ...
  // prevReleases is the PREVIOUS application. Without it a half claimed last month would
  // be counted again in "This Certificate" on this PDF - the screen already passes it, so
  // the document would have disagreed with what the QS was looking at.
  const sum = computeApplicationSummary(app, prevGross, prevReleases)
  ...
  heading('Summary')
  {
    ...
    // RETENTION RELEASE. Only printed when something has been claimed - an unclaimed half
    // is a nil line on a document going to the customer, and this certificate has to be
    // read as a claim rather than a checklist.
    // Both halves shown once either has been released, so a certificate claiming the 2nd
    // still shows the 1st as already released. Otherwise the customer sees a "2nd release"
    // with no sign the first ever happened.
    if (sum.rel1 || sum.rel2) {
      if (sum.rel1) row(`1st Release Retention${sum.rel1New ? '' : ' (released previously)'}`, '', money(sum.release1Value), money(sum.halfRetention))
      if (sum.rel2) row(`2nd Release Retention${sum.rel2New ? '' : ' (released previously)'}`, '', money(sum.release2Value), money(sum.halfRetention))
    }
    y -= 4
  ...
  y -= 6
  {
    ...
    // Same ordering rule as the screen - see the note there.
    if (sum.mcdSplit) {
      cRow('Gross measured work', 'mcdBase')
      cRow(`Main contractor's discount (${app.mcdPct || 0}%)`, 'mcd')
      cRow('Sub-total', 'subTotal')
      cRow(sum.mcdOnVars ? 'Materials on site' : (sum.mcdOnMos ? 'Variations' : 'Variations & materials'), 'after')
      cRow('Sub-total', 'netBeforeRet')
    } else {
    ...
    // Sits between the deduction and the total, which is where it happens: retention
    // comes off, the released half goes back on. Without it the Total on the PDF would
    // not add up from the lines above it - the customer's own check.
    if ((sum.current.released || 0) > 0 || (sum.thisCert.released || 0) > 0) cRow('Retention released', 'released')
  ...
  // 'no' IS NOT false.
  //
  // This tested instructed !== false, and the tracker and the builder both store the
  // string 'no'. 'no' !== false is true, so every UNINSTRUCTED variation was being
  // certified as instructed - a real figure on a document going to the customer, not a
  // cosmetic one.
  //
  // Instructed means exactly true or 'yes'. Anything else - 'no', false, blank, absent -
  // is not instructed, which is the safe direction to be wrong in.
  //
  // This was a local copy while lib/applications.js still had the loose test, so the PDF
  // was right and the on-screen summary was wrong. Now one definition, imported.
  const instructed = vars.filter(isInstructed)
  const notInstructed = vars.filter(v => !isInstructed(v))
  if (instructed.length || notInstructed.length) {
    ...
    // The note pointing at the section at the back. True for an uploaded document OR an
    // instructed variation that will generate its own - computed here rather than reusing
    // varsWithDocs, which is not built until further down.
    const willHaveDocs = (vars || []).some(v =>
      (Array.isArray(v.attachments) && v.attachments.length > 0)
      || (isInstructed(v) && v.builder))
    if (willHaveDocs) {
      ensure(16)
  ...
  // INSTRUCTED VARIATIONS BUILT IN THE PORTAL ATTACH THEMSELVES.
  //
  // Until now a variation only appeared here if somebody had uploaded a PDF of it by
  // hand - which is the manual step you have been doing. Anything built in the Variation
  // Builder can generate its own document, complete with the digital instruction on it,
  // so it does not need to be found and attached.
  //
  // Only INSTRUCTED ones. An uninstructed variation is a proposal; putting it behind an
  // application implies an authority that does not exist.
  //
  // A manually uploaded document still wins - if somebody has attached a signed copy,
  // that is the one the customer should see.
  const withGenerated = await Promise.all((vars || []).map(async (v) => {
    const has = Array.isArray(v.attachments) && v.attachments.length > 0
    if (has || v.instructed !== 'yes' || !v.builder) return v
    try {
      const { buildVariationPDF } = await import('./variationPdf')
      const bytes = await buildVariationPDF({ variation: v, project, logoUrl })
      const b64 = Buffer.from(bytes).toString('base64')
      return {
        ...v,
        attachments: [{
          url: `data:application/pdf;base64,${b64}`,
          name: `Variation ${v.varNumber} - instructed.pdf`,
          generated: true,
        }],
      }
    } catch { return v }
  ...
  const varsWithDocs = withGenerated.filter(v => Array.isArray(v.attachments) && v.attachments.length > 0)
  const groupDocs = []
  ...
  if (varsWithDocs.length > 0 || groupDocs.length > 0) {
    ...
    for (const v of varsWithDocs) {
      // isInstructed, not a truthiness test - instructed is now the string 'no' on an
      // uninstructed variation, and every non-empty string is truthy.
      drawVarHeader(`${v.varNumber || 'VO'} — ${v.description || ''}`, money(variationValue(v)), isInstructed(v) ? 'Instructed' : 'Not yet instructed')
  ...
  const foot = `${san([jobNo, projName].filter(Boolean).join(' - '))}  \u00b7  ${desc.title} ${appNo}  \u00b7  ${fmtDMY(appDate)}`
  pages.forEach((pg, i) => {
...
function buildLiveVars(app, trackerVariations) {
  ...
  return (trackerVariations || []).map(v => {
    ...
    return {
      ...
      //
      // This rebuilt each variation and (a) coerced instructed to a boolean and (b) left
      // the builder block behind. The document generator below tests
      //     v.instructed !== 'yes' || !v.builder
      // so both changes broke it independently, and an instructed variation built in the
      // Variation Builder never had its document - or its digital instruction - appended
      // to the application.
