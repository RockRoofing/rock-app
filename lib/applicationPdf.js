...
export async function buildApplicationPDF({ app, appNumber, project = {}, prevGross = 0, prevReleases = null, trackerVariations = [], logoUrl }) {
  ...
  page.drawText(isFinal ? 'Final Account' : 'Application for Payment', { x: M + (logoImg ? 60 : 0), y: y - 22, size: 20, font: bold, color: INK })
  if (isFinal) {
    // Said twice on purpose. A document titled Final Account that still says
    // "Application no. 14" invites the question; this answers it on the page.
    page.drawText('FINAL ACCOUNT - this is the last application on this project', {
      x: M + (logoImg ? 60 : 0), y: y - 36, size: 9, font: bold, color: INK,
    })
  }
  ...
  const foot = `${san([jobNo, projName].filter(Boolean).join(' - '))}  \u00b7  ${app.isFinalAccount ? 'Final Account' : 'Application'} ${appNo}  \u00b7  ${fmtDMY(appDate)}`
  pages.forEach((pg, i) => {
