...
export async function buildApplicationPDF({ app, appNumber, project = {}, prevGross = 0, trackerVariations = [], logoUrl }) {
  ...
  // Each variation with attachments gets its own header (VO number – Description,
  // Total) followed by the documents attached to that variation. Supplier/material
  // group attachments follow afterwards.
  ...
  const embedAttachment = async (att, headerLabel) => {
    try {
      const url = att.url
      ...
      if (isPdf) {
        ...
        if (headerLabel) p2.drawText(san(headerLabel), { x: M, y: A4[1] - M, size: 10, font: bold, color: GREY })
  ...
  if (varsWithDocs.length > 0 || groupDocs.length > 0) {
    ...
    cover.drawText('Variation Supporting Documents', { x: M, y: A4[1] - M - 20, size: 18, font: bold, color: INK })
    cover.drawLine({ start: { x: M, y: A4[1] - M - 30 }, end: { x: A4[0] - M, y: A4[1] - M - 30 }, thickness: 0.8, color: LINE })
    let cy = A4[1] - M - 55
    cover.drawText('The following pages contain the supporting information for each variation, in order.', { x: M, y: cy, size: 10, font, color: GREY })
    cy -= 24
    for (const v of varsWithDocs) {
      const label = `${v.varNumber || 'VO'} — ${san(v.description || '')}`.slice(0, 95)
      cover.drawText(label, { x: M, y: cy, size: 10, font: bold, color: INK })
      cover.drawText(money(variationValue(v)), { x: A4[0] - M - bold.widthOfTextAtSize(money(variationValue(v)), 10), y: cy, size: 10, font: bold, color: INK })
      cy -= 16
      if (cy < M + 40) break
    }

    // Per-variation: header block, then its documents.
    for (const v of varsWithDocs) {
      const hp = pdf.addPage(A4)
      hp.drawText('Variation Supporting Documents', { x: M, y: A4[1] - M, size: 9, font: italic, color: GREY })
      hp.drawText(`${v.varNumber || 'VO'} — ${san(v.description || '')}`, { x: M, y: A4[1] - M - 28, size: 14, font: bold, color: INK })
      hp.drawText(`Total: ${money(variationValue(v))}`, { x: M, y: A4[1] - M - 48, size: 11, font: bold, color: GOLD })
      hp.drawText(`${v.instructed ? 'Instructed' : 'Not yet instructed'}`, { x: M, y: A4[1] - M - 64, size: 9, font, color: GREY })
      hp.drawLine({ start: { x: M, y: A4[1] - M - 74 }, end: { x: A4[0] - M, y: A4[1] - M - 74 }, thickness: 0.6, color: LINE })
      for (const a of v.attachments) await embedAttachment({ url: a.url, name: a.name }, `${v.varNumber || 'VO'} — ${a.name || ''}`)
    }

    // Supplier / material group documents.
    for (const g of groupDocs) await embedAttachment(g, `${g.label} — ${g.name || ''}`)
  }
