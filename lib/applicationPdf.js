...
export async function buildApplicationPDF({ app, appNumber, project = {}, prevGross = 0, trackerVariations = [], logoUrl }) {
  ...
  if (varsWithDocs.length > 0 || groupDocs.length > 0) {
    ...
    const pdfTail = []   // {label, bytes} PDFs to append at the very end
    const drawAttachment = async (att, caption) => {
      try {
        ...
        if (isPdf) {
          // Can't scale PDF pages inline reliably; note it here and append at the end.
          need(16)
          sp.drawText(san(`• ${caption || att.name || 'Document'} — see appended pages`), { x: M + 4, y: sy - 10, size: 9, font: italic, color: GREY })
          sy -= 16
          pdfTail.push({ label: caption || att.name || 'Document', bytes })
          return
        }
    ...
    // Append any multi-page PDF attachments as full pages at the very end, each with
    // a small header naming which variation/document they belong to.
    for (const t of pdfTail) {
      try {
        const src = await PDFDocument.load(t.bytes, { ignoreEncryption: true })
        const copied = await pdf.copyPages(src, src.getPageIndices())
        copied.forEach((pg, i) => {
          pdf.addPage(pg)
          if (i === 0) pg.drawText(san(t.label), { x: M, y: pg.getHeight() - 16, size: 8, font: italic, color: GREY })
        })
      } catch { /* skip */ }
    }
  }
