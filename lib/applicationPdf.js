...
export async function buildApplicationPDF({ app, project = {}, prevGross = 0, trackerVariations = [], logoUrl }) {
  ...
  heading('Contract Works')
  {
    ...
    for (const r of cw) {
      if (r.kind === 'heading') { ensure(15); page.drawText(san(r.description), { x: cCode, y: y - 10, size: 9, font: bold, color: INK }); y -= 15; continue }
      ensure(14)
    ...
    page.drawLine({ start: { x: M, y }, end: { x: A4[0] - M, y }, thickness: 0.6, color: LINE }); y -= 4
