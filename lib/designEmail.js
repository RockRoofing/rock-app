...
// Match the rest of the app's working senders: FORMS_FROM_EMAIL is the verified sending
// address used by every other email in the portal. (NOTIFY_FROM_EMAIL was tried first
// before and, if set to an unverified domain, caused every design email to fail silently.)
...
// Notify someone about a new comment / activity on an item (RFI, Tech Sub, Drawing...).
// `cta` sets the button label so it names the right thing (defaults to Review the RFI).
export async function sendRfiCommentNotice({ to, recipientName, projectNo, projectName, rfiNumber, authorName, commentHtml, rfiLink, mentioned, cta }) {
  ...
  const html = shell(`
    ...
    ${btn(rfiLink, cta || 'Review the RFI')}
    <p style="font-size:13px;color:#666">Link: <a href="${rfiLink}">${rfiLink}</a></p>
  `)
  return send({ to, subject, html })
}

// Notify that a new RFI has been raised (says who from Rock Roofing raised it).
export async function sendRfiIssuedNotice({ to, recipientName, raisedByName, projectNo, projectName, rfiNumber, description, requiredDate, rfiLink }) {
  const subject = `New RFI ${rfiNumber} - ${projectName || projectNo}`
  const html = shell(`
    <h2>Hi ${esc(recipientName) || 'there'},</h2>
    <p><strong>${esc(raisedByName) || 'Someone'} from Rock Roofing</strong> has raised a new Request for Information, <strong>${esc(rfiNumber)}</strong>, for project <strong>${esc(projectName || projectNo)}</strong>.</p>
...
// Notify a customer that the O&M Manual is ready to view/download. Uses the Rock Roofing
// green brand colour for the button (the O&M manual uses green, not the portal purple).
export async function sendOmReadyNotice({ to, recipientName, projectNo, projectName, omLink, senderName }) {
  const subject = `O&M Manual ready - ${projectName || projectNo}`
  const greenBtn = (href, label) => `<p style="text-align:center;margin:24px 0"><a href="${href}" style="background:#1c704f;color:#fff;text-decoration:none;padding:13px 26px;border-radius:10px;font-weight:600;display:inline-block">${label}</a></p>`
  const html = shell(`
    <h2>Hi ${esc(recipientName) || 'there'},</h2>
    <p>The <strong>Operation &amp; Maintenance Manual</strong> for project <strong>${esc(projectName || projectNo)}</strong> is now ready to view and download.</p>
    <p>Click below to open the project portal and download it${senderName ? `. Sent by ${esc(senderName)} at Rock Roofing` : ''}.</p>
    ${greenBtn(omLink, 'View &amp; download the O&M Manual')}
    <p style="font-size:13px;color:#666">Link: <a href="${omLink}">${omLink}</a></p>
  `)
  return send({ to, subject, html })
}
