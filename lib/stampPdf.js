...
// Overall stamp size. 0.7 = 30% smaller than the original sizing.
const STAMP_SCALE = 0.7
...
export async function stampPdfBytes(srcBytes, stamps) {
  ...
  // Scale the stamp with the sheet so it reads the same on A4 and A0. STAMP_SCALE shrinks
  // the stamp itself without pulling it closer to the corner (the margin stays put).
  const pageScale = Math.max(1, Math.min(3, Wv / 595.28))
  const scale = pageScale * STAMP_SCALE
  const pad = 9 * scale
  const labelSize = 13 * scale
  const dateSize = 8.5 * scale
  const lineGap = 4 * scale
  const stackGap = 8 * scale
  const margin = 18 * pageScale
