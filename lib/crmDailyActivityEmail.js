...
// STAGES THAT ARE NOT EXPECTED TO HAVE AN ACTIVITY.
//
// Project In is the inbox - a lead that has arrived and not yet been looked at. Nobody
// has taken it on, so there is nothing to chase, and listing it every morning trained
// people to skim the email rather than read it. A warning that is usually noise gets
// ignored, and then the real ones go with it.
//
// It reappears the moment somebody moves it to 1st Contact, which is the point at which
// it becomes somebody's job.
const NO_ACTIVITY_EXEMPT_STAGES = new Set(['stage_project_in'])

// Open projects with nothing outstanding against them. These are the ones that quietly go
// cold, so the whole pre-contract team gets the same list rather than it being one
// person's job to notice.
export async function collectProjectsWithoutActivity() {
  const [open, deals] = await Promise.all([
    get(OPEN_ACTIVITIES).then((v) => (Array.isArray(v) ? v : [])),
    get(DEALS_KEY).then((v) => (Array.isArray(v) ? v : [])),
  ])
  const hasOpen = new Set(open.map((a) => String(a.dealId)))
  return deals
    .filter((d) => d.status === 'open' && !NO_ACTIVITY_EXEMPT_STAGES.has(d.stageId) && !hasOpen.has(String(d.id)))
    .map((d) => ({
      id: d.id,
      title: d.title || '(untitled)',
      company: d.fields?.organization || '',
      stage: d.stageId || '',
      owner: d.fields?.estimator_responsible || d.fields?.sales_person || '',
    }))
    .sort((a, b) => String(a.title).localeCompare(String(b.title)))
}

export async function sendNoActivityEmail({ dryRun, stageLabels = {} } = {}) {
  const [projects, portal] = await Promise.all([collectProjectsWithoutActivity(), getPortalUsers()])

  const recipients = (portal || [])
    .filter((u) => u.active !== false && u.email)
    .filter((u) => ['pre-contract', 'admin'].includes(normRole(u.role)))
    .map((u) => ({ name: [u.firstName, u.lastName].filter(Boolean).join(' ') || u.name || '', email: u.email }))

  if (!projects.length) return { ok: true, sent: 0, note: 'Every open project has an activity set.' }
  if (!recipients.length) return { ok: false, error: 'No pre-contract or admin users with an email address.' }
  if (dryRun) return { ok: true, dryRun: true, projects: projects.length, wouldSend: recipients.map((r) => r.email) }

  const key = process.env.RESEND_API_KEY
  if (!key) return { ok: false, error: 'Email not configured (RESEND_API_KEY)', projects: projects.length }
  ...
  const rows = projects.map((p) => `<tr style="border-top:1px solid #eee">
      <td style="padding:7px 10px 7px 0"><div style="font-weight:600;color:#1a1a19">${esc(p.title)}</div>
        <div style="color:#888;font-size:12px">${esc(p.company)}</div></td>
      <td style="padding:7px 10px 7px 0;color:#666;white-space:nowrap">${esc(stageLabels[p.stage] || '')}</td>
      <td style="padding:7px 0;color:${p.owner ? '#666' : '#b45309'};white-space:nowrap">${esc(p.owner || 'nobody assigned')}</td>
    </tr>`).join('')

  const html = `
    <div style="font-family:system-ui,Arial,sans-serif;max-width:680px;color:#1a1a19">
      <h2 style="margin:0 0 4px">Projects with no activity set</h2>
      <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:12px 14px;margin:14px 0;font-size:14px;color:#9a3412">
        <strong>All projects must have an activity set.</strong> Please check your projects and update accordingly.
      </div>
      <p style="color:#666;font-size:13px;margin:0 0 6px">${projects.length} open project${projects.length === 1 ? '' : 's'} with nothing outstanding:</p>
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        <tr style="text-align:left;color:#888;font-size:11px">
          <th style="padding-bottom:4px">Project</th><th style="padding-bottom:4px">Stage</th><th style="padding-bottom:4px">Owner</th>
        </tr>
        ${rows}
      </table>
      <p style="margin-top:22px"><a href="${baseUrl}/crm" style="background:#2a7de1;color:#fff;text-decoration:none;padding:9px 16px;border-radius:8px;font-size:13px;font-weight:600">Open the CRM</a></p>
  ...
  for (const r of recipients) {
    try {
      const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({ from, to: [r.email], subject: `CRM: ${projects.length} project${projects.length === 1 ? '' : 's'} with no activity set`, html }),
      })
      if (res.ok) sent++; else failed.push(r.email)
    } catch { failed.push(r.email) }
  }
  return { ok: true, sent, failed, projects: projects.length }
}
