...
export function deriveMilestones(crmDeals) {
  ...
  for (const d of (Array.isArray(crmDeals) ? crmDeals : [])) {
    ...
    // First entry into Received. A project CREATED at Received counts: createProject now
    // writes a stage entry with stageFrom empty, so creating one straight into Received
    // reads as entering it - which it is. Before that there was no move to find, so a
    // project added directly at Received never appeared on Tenders Received at all.
    ...
    // Reaching Received is not only a move INTO it. A project created straight into
    // Received before pkg405 recorded no stage entry at all, and one created there and
    // then moved ON only ever recorded the move AWAY. Both were left with no received
    // date and could never appear on the Glenigan cards however they were scored.
    //
    // So: a recorded move OUT of Received proves it was there beforehand, and being at
    // Received or beyond right now proves it got there somehow.
    const RECEIVED_OR_LATER = ['stage_received', 'stage_1', 'stage_2', 'stage_review',
      'stage_mc_unsecured', 'stage_variations', 'stage_mc_secured', 'stage_negotiating']
    let leftReceived = null
    for (const h of ordered) {
      if (h.type !== 'stage') continue
      const from = h.stageFrom || (/Stage:\s*(.+?)\s*(?:->|\u2192)/.exec(String(h.text || '')) || [])[1]
      if (from && String(from).trim() === RECEIVED_LABEL) { leftReceived = dayOf(h.ts); break }
    }
    const inReceivedNow = RECEIVED_OR_LATER.includes(d.stageId)
    ...
    // Last resort for a deal we KNOW has been received but has no recorded move: use the
    // day it was created. Only for deals created since the CRM went live - an imported
    // project reached Received long before, and inventing a date for it would be a lie
    // dressed as data.
    const createdIsRecent = !!createdDate && createdDate >= CRM_LIVE_FROM
    if (!receivedDate && (inReceivedNow || leftReceived) && createdIsRecent) {
      receivedDate = leftReceived && leftReceived < createdDate ? leftReceived : createdDate
    }

    if (receivedDate || inReceivedNow || everInProjectIn) {
      out[String(d.id)] = {
        receivedDate: receivedDate || null,
        everInReceived: !!(receivedDate || inReceivedNow || leftReceived),
        projectInDate,
        everInProjectIn,
      }
    }
