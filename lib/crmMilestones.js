...
export function deriveMilestones(crmDeals) {
  ...
  for (const d of (Array.isArray(crmDeals) ? crmDeals : [])) {
    ...
    for (const h of ordered) {
      if (h.type !== 'stage') continue
      const to = h.stageTo || (/(?:->|\u2192)\s*(.+)$/.exec(String(h.text || '')) || [])[1]
      if (!to || String(to).trim() !== PROJECT_IN_LABEL) continue
    ...
    if (createdDate && (!projectInDate || createdDate < projectInDate)) projectInDate = createdDate

    const everInProjectIn = !!projectInDate || d.stageId === PROJECT_IN_STAGE_ID
