...
async function buildMonth(monthKey, projects, projectSet) {
  ...
  const ids = projectSet || projects.filter((p) => p.stage === 'INPROGRESS').map((p) => String(p.xeroId))
...
export async function refreshGpSnapshots(projects, { months = MONTHS_BACK, only = '' } = {}) {
  ...
  for (const mk of targets) {
    ...
    const built = await buildMonth(mk, projects, existing ? existing.projects : null)
    if (!existing) built.projectSetFrom = 'live at first snapshot'
    else built.projectSetFrom = existing.projectSetFrom || 'reused'
    store[mk] = built
    done.push({ month: mk, projects: built.projects.length, estimators: Object.keys(built.byEstimator).length, reusedSet: !!existing })
  }
  await set(KEY, store)
