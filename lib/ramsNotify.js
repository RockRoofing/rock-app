...
export async function notifySiteManagerRejection({ projectNo, fileName, byName, notes }) {
  try {
    const p = await projectByNo(projectNo); if (!p) return
    const label = projLabel(p)
    const recipients = []
    const cm = await findProjectCM(p); if (cm?.email) recipients.push(cm.email)
    let director = (await get('ops:rams-director')) || null
    if (!director?.email) {
      try { const portal = (await get('portal:users')) || []; const d = portal.find(u => (u.jobRole || '') === 'Director' && u.active !== false && u.email); if (d) director = { email: d.email } } catch {}
    }
    if (director?.email) recipients.push(director.email)
