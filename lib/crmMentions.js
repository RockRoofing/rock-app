...
export async function notifyMentions({ body, author, users, what = 'a comment', context = '', link = '', cta = 'Open it' }) {
  ...
  const self = matched.filter((u) => author && u.name.toLowerCase() === String(author).toLowerCase())
  // Nobody needs an email telling them what they just typed.
  const targets = matched.filter((u) => !author || u.name.toLowerCase() !== String(author).toLowerCase())
