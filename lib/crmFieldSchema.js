...
export const DEFAULT_FIELD_SCHEMA = [
  ...
  // A DROPDOWN, not free text. The Glenigan cards read this with parseInt, so "5 - good"
  // or a stray space scored nothing and looked like a broken metric rather than a typo.
  // 0 to 10, and nothing else is possible.
  { key: 'project_score', label: 'Project Score', type: 'select', group: 'summary',
  ...
  { key: 'lead_source', label: 'Lead Source', type: 'select', group: 'details',
    options: ['Glenigan','Website New Build','Website Refurb','Actively Chased','Other'] },

  // WHY A DEAL WAS LOST. The Sales dashboard has a whole Lost Reasons panel built on
  // this, and it was imported but never made editable - so anything lost IN the CRM had
  // no reason and the panel would have degraded quietly, exactly as the close date did.
  //
  // These are the reasons actually in use, from the export: the 15 below cover 99% of
  // 6,005 lost deals. The remaining one-offs are free text and are preserved - the
  // select keeps any stored value that is not on this list rather than blanking it.
  { key: 'lost_reason', label: 'Lost Reason', type: 'select', group: 'details',
    options: ['Roofing package already let','Price','Package Contractor','Unable to make contact',
      'Alternative spec','Project Shelved','Project Not Suitable','Gone with preferred, approved suppy chain',
      'No Feedback Given','Main Contractor not suitable',"Main Contractor didn't secure",'No Time to Price',
      'No available labour resource','Lead Time','Design Liability','Duplicate','No Single Ply'] },
