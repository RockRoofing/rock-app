...
export async function scan(cursor, opts) {
  ...
  return [0, Object.keys(store).filter(k => k.startsWith(p))]
}
...
export async function getClient() {
  ...
  return {
    get: async (k) => (store[k] === undefined ? null : store[k]),
    set: async (k, v) => { store[k] = v },
    del: async (k) => { delete store[k]; return 1 },
    keys: async (p) => Object.keys(store).filter(k => k.startsWith(String(p).replace('*', ''))),
    scan: async (cursor, opts) => {
      const match = (opts && opts.match) || '*'
      const pre = match.replace('*', '')
      return [0, Object.keys(store).filter(k => k.startsWith(pre))]
    },
  }
}
