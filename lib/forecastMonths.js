...
export function yearEndWip({ cf, rows, months, fyEnd, blendedMargin, enabled, override }) {
  ...
  // Only where November is still FORECAST. Once it is switched to Actual, Xero's own WIP
  // journal is inside the income figure and the include-at-year-end rule handles it -
  ...
  for (const fc of (cf.projForecasts || [])) {
    const a = fc.accrual
    if (!a || !fc.from || !fc.to) continue
    if (fc.latestAppEnd && fc.to && fc.to <= fc.latestAppEnd) continue
    // The period that STRADDLES the year end.
    if (!(fc.from <= monthEnd && fc.to > monthEnd)) continue

    const matIn = (a.materials || [])
      .filter(x => x.date && x.date >= fc.from && x.date <= monthEnd)
      .reduce((t, x) => t + (Number(x.amount) || 0), 0)

    const labTotal = (a.labour || []).reduce((t, x) => t + (Number(x.amount) || 0), 0)
    const wdBefore = workingDays(fc.from, monthEnd)
    const wdAll = workingDays(fc.from, fc.to)
    const labAccrued = wdAll > 0 ? labTotal * (wdBefore / wdAll) : 0

    const accrued = matIn + labAccrued
    if (accrued <= 0) continue

    // What this SAME forecast has already contributed to November, by the ordinary
    // bucketing rule. Anything already there must not be added again.
    const inNov = (x) => x.date ? String(x.date).slice(0, 7) === novKey : false
    const alreadyIn = (a.materials || []).filter(inNov).reduce((t, x) => t + (Number(x.amount) || 0), 0)
      + (a.labour || []).filter(inNov).reduce((t, x) => t + (Number(x.amount) || 0), 0)

    projects.push({
      name: projLabel(fc), from: fc.from, to: fc.to, valDate: fc.valDate || '',
      materials: matIn, labourTotal: labTotal, labour: labAccrued,
  ...
  return {
    ...
    reason: projects.length ? '' : 'no project period straddles 30 November - the forecasts may have run out before the year end',
  }
}
