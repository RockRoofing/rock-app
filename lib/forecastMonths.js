...
export function yearEndWip({ cf, rows, months, fyEnd, blendedMargin, enabled, override }) {
  ...
  for (const fc of (cf.projForecasts || [])) {
    ...
    // THE FORECAST MUST STILL BE RUNNING AT MONTH END.
    //
    // This test existed, and I removed it when rebuilding around application dates. A
    // Renshaw period running 14 to 30 September then got a window from its own start
    // through to 30 November - two and a half months of cost in what should be a
    // few days. A period that finished in September has no unclaimed work at the year
    // end; it was claimed in September.
    if (!fc.from || !fc.to) continue
    if (fc.from > monthEnd || fc.to < monthEnd) continue
    ...
    // LABOUR: ONLY THE STRADDLING INSTALMENT IS PRO-RATED.
    //
    // A labour line carries the END of its instalment window. A line dated ON OR BEFORE
    // the 30th is wholly within the window and counts at FACE VALUE - pro-rating it was
    // what made "already in Nov" exceed the accrued cost, which cannot be true.
    //
    // Only the first line dated AFTER month end straddles the boundary. Its window runs
    // from the previous line's date, so the share falling on or before the 30th is the
    // working days in that part over the working days in the whole instalment.
    const labLines = (a.labour || []).filter(x => x.date).slice().sort((x, y) => x.date < y.date ? -1 : 1)
    let labWhole = 0, labPart = 0, wdBefore = 0, wdAll = 0
    for (let i = 0; i < labLines.length; i++) {
      const x = labLines[i]
      const amt = Number(x.amount) || 0
      if (!amt) continue
      if (x.date >= windowStart && x.date <= monthEnd) { labWhole += amt; continue }
      if (x.date > monthEnd && (!nextApp || x.date <= nextApp) && !labPart) {
        const prev = i > 0 ? labLines[i - 1].date : null
        const instStart = prev && prev >= windowStart ? addDay(prev) : windowStart
        if (instStart > monthEnd) continue
        wdBefore = workingDays(instStart, monthEnd)
        wdAll = workingDays(instStart, x.date)
        if (wdAll > 0) labPart = amt * (wdBefore / wdAll)
      }
    }
    const labAccrued = labWhole + labPart

    const accrued = matIn + labAccrued
    if (accrued <= 0.5) continue

    // Already bucketed into November by the ordinary rule: everything DATED in the
    // window. The top-up is therefore exactly the straddling share, and can never be
    // negative.
    const alreadyIn = matIn + labWhole

    projects.push({
      name: projLabel(fc), from: windowStart, to: monthEnd,
      lastApp: lastApp || '(none before month end)', nextApp: nextApp || '',
      valDate: fc.valDate || '',
      materials: matIn, labourWhole: labWhole, labourPart: labPart, labour: labAccrued,
