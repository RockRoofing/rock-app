...
async function countSentExternal({ mailbox, from, to, max = 2000 }) {
...
export async function refreshEmailVolume({ months = 2, mailbox = '', max = 2000 } = {}) {
  ...
  for (const b of boxes) {
    try {
      const { byMonth, seen } = await countSentExternal({ mailbox: b.email, from, to, max })
      ...
      // A month inside the window with no external mail is a real zero, not a gap.
      ...
      result.mailboxes.push({ mailbox: b.email, scanned: seen, byMonth })
    } catch (e) {
