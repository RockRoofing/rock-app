...
async function drawSectionCover(doc, { number, title, pageNo, fonts }) {
  ...
  page.drawRectangle({ x: 0, y: A4[1] - 210, width: 10, height: 210, color: PURPLE })
  page.drawText(`SECTION ${number}`, { x: 56, y: A4[1] - 120, size: 16, font: bold, color: PURPLE })
  ...
  page.drawText(`Page ${pageNo}`, { x: 56, y: A4[1] - 250, size: 13, font: reg, color: GREY })
  // Footer
  page.drawText('Operation & Maintenance Manual', { x: 56, y: 40, size: 8, font: reg, color: GREY })
}

export async function buildOMManual({ project, sections, logoUrl, rockRoofing, mainContractor }) {
  ...
  cover.drawRectangle({ x: 0, y: 0, width: W, height: 14, color: PURPLE })
  cover.drawRectangle({ x: 0, y: H - 14, width: W, height: 14, color: PURPLE })
  let cy = H - 70
  if (logoUrl) {
    try {
  ...
  for (const ln of nameLines) { const tw = bold.widthOfTextAtSize(ln, 20); cover.drawText(ln, { x: (W - tw) / 2, y: cy, size: 20, font: bold, color: PURPLE }); cy -= 26 }
  cy -= 6
  ...
  const drawCompanyBox = (x, heading, lines) => {
    ...
    cover.drawText(heading, { x: x + 14, y: boxY + boxH - 24, size: 11, font: bold, color: PURPLE })
    let ly = boxY + boxH - 46
  ...
  toc2.drawRectangle({ x: 0, y: H - 90, width: 10, height: 90, color: PURPLE })
  ...
  ry -= 26
  for (const t of toc) {
    toc2.drawText(`${t.number}.`, { x: M, y: ry, size: 12, font: bold, color: PURPLE })
  ...
  for (let p = 0; p < pages.length; p++) {
    ...
    if (n >= 2) {
      ...
      pages[p].drawText(label, { x: W - M - reg.widthOfTextAtSize(label, 9), y: 24, size: 9, font: reg, color: GREY })
    }
  }
