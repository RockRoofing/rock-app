...
export async function buildVariationPDF({ variation, project, logoUrl }) {
  ...
  // Photos and documents attached in the builder. The key is still called `photos`
  // because that is what pkg817 wrote and variations already hold it - renaming it
  // would strand those. Kept on the builder block rather
  ...
  // instruction email - builds it here, so they all carry the attachments without any
  // of them needing to know about them.
  //
  // Images are drawn onto the page. PDFs have their pages COPIED IN whole, so a
  // supplier quote or a marked-up drawing arrives intact rather than as a link the
  // customer has to go and find.
  if (photos.length) {
    // A fresh page. Evidence on the end of a half-full page of pricing looks like an
    // afterthought and the first item gets squeezed.
    page = pdf.addPage([W, H]); y = H - M
    page.drawText('Attachments', { x: M, y, size: 8.5, font: bold, color: INK })
    y -= 16

    const isPdfFile = (ph) => /\.pdf($|\?|#)/i.test(String(ph.url || ''))
      || String(ph.name || '').toLowerCase().endsWith('.pdf')
      || String(ph.url || '').startsWith('data:application/pdf')

    const note = (text, color) => {
      if (y < M + 20) { page = pdf.addPage([W, H]); y = H - M }
      page.drawText(san(text), { x: M, y, size: 7.5, font, color })
      y -= 14
    }
    ...
    for (const ph of photos) {
      let bytes = null
      try {
        bytes = String(ph.url).startsWith('data:')
          ? Uint8Array.from(Buffer.from(String(ph.url).split(',')[1], 'base64'))
          : new Uint8Array(await (await fetch(ph.url)).arrayBuffer())
      } catch {
        // A file that will not load must not take the whole document down with it,
        // and it must not vanish silently either - a variation quietly missing its
        // evidence, with nothing to say any was meant to be there, is worse than a
        // line admitting it.
        note(`[${ph.name || 'attachment'} could not be loaded]`, RED)
        continue
      }
      ...
      if (isPdfFile(ph)) {
        try {
          ...
          // A caption on the first copied page, so a customer can tell which
          // attachment they are looking at once it is all one document.
          copied.forEach((pg, idx) => {
            pdf.addPage(pg)
            if (idx === 0 && ph.name) {
              try { pg.drawText(san(ph.name), { x: 24, y: pg.getHeight() - 16, size: 7.5, font, color: GREY }) } catch { /* odd page box */ }
            }
          })
          // Carry on below the embedded document rather than trying to draw onto its
          // last page, which could be any size or orientation.
          page = pdf.addPage([W, H]); y = H - M
        } catch {
          note(`[${ph.name || 'document'} could not be read as a PDF]`, RED)
        }
        continue
      }

      let img = null
      // Try JPEG then PNG. A photo off a phone is one or the other, and pdf-lib needs
      // to be told which.
      try { img = await pdf.embedJpg(bytes) } catch { try { img = await pdf.embedPng(bytes) } catch { img = null } }
      if (!img) {
        // Not an image and not a PDF - a Word file or a spreadsheet. It cannot be
        // drawn into this document, so it is listed by name rather than dropped.
        note(`[${ph.name || 'file'} - attached to the variation, not shown here]`, GREY)
        continue
      }
      const caption = san(ph.name || '')
      // Scale to the page width, and cap the height so two portrait images can share
