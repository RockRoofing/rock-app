...
export async function buildVariationPDF({ variation, project, logoUrl }) {
  ...
  items.forEach((it, i) => {
    if (y < M + 120) { page = pdf.addPage([W, H]); y = H - M; headRow() }
    page.drawRectangle({ x: M, y: y - 5, width, height: 16, borderColor: LINE, borderWidth: 0.5 })
    row([String(i + 1), it.description || '', String(Number(it.qty) || ''), it.unit || '', money(it.rate), money(it.total)], font, 7.5, INK, y)
    y -= 16
  })
