...
export function resolveGrossAfa(opts) {
  ...
  if (stampIsFromLatestSent) {
    return { afa: Number(stamp.afaOverride), source: `application ${label} (sent, as issued)`,
             fromApp: true, stamp, stampIsFromLatestSent: true, latestSent }
  }
  ...
  if (latestSent) {
    ...
    const v = sum && sum.anticipatedFinalAccount
    if (v != null && isFinite(v) && v > 0) {
      return { afa: Number(v), source: `application ${label} (sent)`,
               fromApp: true, stamp, stampIsFromLatestSent: false, latestSent, sum }
    }
  }

  if (stamp) {
    return { afa: Number(stamp.afaOverride), source: 'stored override (earlier application)',
             fromApp: false, stamp, stampIsFromLatestSent: false, latestSent }
  }

  return { afa: (Number(o.contractValue) || 0) + (Number(o.instructedVarsTotal) || 0),
           source: 'project details', fromApp: false, stamp: null, stampIsFromLatestSent: false, latestSent }
}
