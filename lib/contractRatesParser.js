...
export function parseTakeOffRows(rows) {
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    // Headers are NOT auto-detected any more. Every coded row comes in as a plain
    // item (blank qty/rate/total left blank); the user manually marks a row as a
    // header (bold) on the Contracted Rates page if they want one.
    const kind = 'item'
