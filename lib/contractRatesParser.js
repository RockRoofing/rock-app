...
export function parseTakeOffRows(rows) {
  ...
  const items = []
  const skipped = []   // rows we did NOT import, with a reason (surfaced to the user)
  let below = false
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    // Does this row carry any real item data (beyond just a description)?
    const hasData = qty != null || rate != null || total != null || matRate != null || labRate != null || totalIsText

    // The "BELOW THE LINE ITEMS" divider flips us into the optional/variation
    // section. Match it PRECISELY - only a row that is essentially just that label
    // (no item data) is the divider. A real item whose description merely mentions
    // "below the line" must NOT be treated as the divider (that used to swallow it).
    if (descU.includes('BELOW THE LINE') && !hasData) { below = true; continue }
    // Summary/total rows we never import.
    if (descU.includes('GROSS MARGIN SUMMARY')) continue
    if (/^(sub\s*)?total\b/i.test(desc) && !code) continue

    // Import rule: a row is a real item if it has an item code in column A, OR it
    // has no code but clearly carries item data (description + qty/rate/total).
    // This catches lines where the Schedule ID cell is blank or merged (Excel puts
    // the merged value only on the first row, leaving continuation rows blank) -
    // previously these were dropped silently.
    const isItem = !!code || (desc && hasData)
    if (!isItem) {
      // Only report rows that look like they might have been meant as items
      // (they have a description) so the user can see what was left out.
      if (desc && (hasData || descU.length > 0)) {
        // Pure section headings (description only, no data) are normal - don't nag.
        if (hasData) skipped.push({ row: i + 1, code, description: desc, reason: 'No item code and could not confirm as an item' })
      }
      continue
    }
    ...
    items.push({
      ...
      kind,
      struck: false,
      codeMissing: !code,   // flag so the page can highlight rows that came in without a code
    })
  }
