...
export function overheadEvents(schedule, budgets, start, end, predictedByCodeMonth) {
  ...
  // EVERY OVERHEAD CODE, not just the scheduled ones.
  //
  // This iterated the SCHEDULE, so a code with no cash schedule set produced no events at
  // all and simply never left the bank - while the Budgets page counted it every month.
  // That is why the cash flow and Budgets disagreed by a different amount each month
  // rather than a constant.
  //
  // A code without a schedule now pays its predicted figure on the 28th, which is the same
  // default the 'oneday' mode uses. Codes WITH a schedule are untouched and still pay on
  // their own dates - that part was always right, and is why a cash flow month can
  // legitimately differ from the budget month.
  const scheduledCodes = new Set(Object.keys(schedule || {}).filter(c => (schedule[c] || {}).mode))
  const allCodes = new Set([
    ...scheduledCodes,
    ...Object.keys(predictedByCodeMonth || {}),
    ...Object.keys(budgets || {}),
  ])
  for (const code of allCodes) {
    const sc = (schedule || {})[code]
    if (!sc || !sc.mode) {
      for (const mDate of months) {
        const y = mDate.getFullYear(), m = mDate.getMonth()
        const mk = `${y}-${pad(m + 1)}`
        const amt = amountFor(code, mk, null)
        if (Math.abs(amt) < 0.005) continue
        events.push({ date: `${mk}-${pad(clampDay(y, m, 28))}`, amount: amt, code })
      }
      continue
    }
