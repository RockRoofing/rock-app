...
export async function buildOutstandingInvoicesPDF({ invoices, includeComments = true, logoUrl, title = 'Outstanding Invoices' }) {
  ...
  page.drawText(title, { x: M + (logoImg ? 50 : 0), y: y - 22, size: 18, font: bold, color: INK })
  y -= lh + 4
  ...
  if (includeComments) {
    for (const inv of invoices) {
      newPage()
      // title
      page.drawText(`Invoice ${inv.invoiceNumber || ''}`, { x: M, y: y - 18, size: 15, font: bold, color: INK })
      if (inv.highRisk) {
        const t = 'HIGH RISK'
      ...
      for (const [k, v] of rows) {
        ...
        page.drawText(k, { x: M, y: y - 11, size: 9.5, font: bold, color: GREY })
        page.drawText(String(v), { x: M + 110, y: y - 11, size: 9.5, font, color: (k === 'Due date' && inv.overdueBy) ? RED : INK })
        y -= 15
      }
      y -= 6
      ...
      if (!comments.length) text('No comments recorded.', { color: GREY, size: 9, f: italic })
        ...
        page.drawText(`${c.author || 'Unknown'}${c.source === 'email-bcc' ? ' (via email)' : ''}`, { x: M, y: y - 10, size: 9, font: bold, color: INK })
        const ds = fmtDT(c.at)
