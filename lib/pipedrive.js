...
// One slot per customer - see lib/scopedCache.js.
const pipelineIdCache = createScopedCache()
export async function getProjectPipelineId() {
  const cached = pipelineIdCache.get()
  if (cached) return cached
  const res = await fetch(`${BASE}/pipelines?api_token=${KEY()}`)
  const data = await res.json()
  const pipeline = data.data?.find(p => p.name.trim() === 'Project')
  return pipelineIdCache.set(pipeline?.id || null)
}
...
export function normaliseDeal(d, fieldMap, wonOrgCounts = {}) {
  ...
  return {
    ...
    // Negotiating page custom fields (added by key)
    siteLocation: d['31507bc0e875f8f426274e66c6ace262d34a5d0b'] || null,
    sizeM2: d['3162812241fa2f57765894ec4c9899b66a60e90d'] || null,
    creditScore: d['a2b502a50924d2359b8201a627268befdf2c79a2'] || null,
    creditLimit: d['53ab9aae2ce7c8ff1b6a829290ec784567681786'] || null,
    insuredCreditLimit: d['8acf6f9e21e28beb80bb5a6ca045592cf4718dfb'] || null,
    scopeOfWorks: d['c5ad7b511a95767ed7c34eb27bf5c29246c2b789'] || null,
