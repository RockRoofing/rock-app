...
export function rockDrawingDigestHtml({ name, projectName, projectNo, comments }) {
  const n = comments || 0
  const link = `${APP_URL}/design/${encodeURIComponent(projectNo)}/rock-drawings`
  return shell(`
    <h2>Hi ${esc(name) || 'there'},</h2>
    <p>There ${n === 1 ? 'has been a new comment' : 'have been new comments'} added to Rock Drawings for your project <strong>${esc(projectName || projectNo)}</strong> today.</p>
...
export function calculationDigestHtml({ name, projectName, projectNo, comments }) {
  const n = comments || 0
  const link = `${APP_URL}/design/${encodeURIComponent(projectNo)}/calculations`
  return shell(`
    <h2>Hi ${esc(name) || 'there'},</h2>
    <p>There ${n === 1 ? 'has been a new comment' : 'have been new comments'} added to Calculations for your project <strong>${esc(projectName || projectNo)}</strong> today.</p>
