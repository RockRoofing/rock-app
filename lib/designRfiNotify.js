...
export function dailyDigestHtml({ name, projectName, projectNo, newRfis, comments }) {
  const lines = []
  if (newRfis > 0) lines.push(`there ${newRfis === 1 ? 'has been a new RFI' : 'have been new RFIs'} added for your project.`)
  if (comments > 0) lines.push(`there ${comments === 1 ? 'has been a comment' : 'have been comments'} added to RFIs for your project.`)
  return shell(`
    <h2>Hi ${esc(name) || 'there'},</h2>
    <p>On project <strong>${esc(projectName || projectNo)}</strong>, ${lines.join(' Also, ')}</p>
...
function rfiTableHtml(rfis, personName) {
  const rows = rfis.map(r => {
    ...
    return `<tr>
      ...
      <td style="padding:7px 10px;border-bottom:1px solid #eee;white-space:nowrap">${esc(req)}</td>
