...
export default function RiskLog() {
  ...
  async function saveMeta() {
    await fetch('/api/risks', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'meta', id: metaEdit.id, patch: { assignee: metaEdit.assignee, closeOutDate: metaEdit.closeOutDate, closed: metaEdit.closed } }) })
  ...
  return (
    <OperationsShell active="risks" title="Risk Log" wide>
              ...
              {filtered.map(r => (
                <tr key={r.id} style={{ borderTop: '1px solid #f0f0f0', verticalAlign: 'top', background: r.closed ? '#fafefb' : '#fff' }}>
                  ...
                  <td style={td}>{r.closed
                    ...
                    {r.source === 'ihm' && <span title="From Internal Handover Minutes" style={{ marginLeft: 6, fontSize: 10, color: '#aaa' }}>IHM</span>}
                  </td>
                  <td style={{ ...td, textAlign: 'right', whiteSpace: 'nowrap' }}>
                    <button onClick={() => setMetaEdit({ id: r.id, assignee: r.assignee, closeOutDate: r.closeOutDate, closed: r.closed })} style={linkBtn}>Update</button>
                    {r.source === 'manual' && <>
