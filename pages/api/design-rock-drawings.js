...
import { rdRecordPendingComment, rdRecordPendingDoc, rdGetReadMap, rdMarkRead, rdUnread, projectDisplayName } from '../../lib/designRfiNotify'
...
// Rock Roofing can additionally mark a drawing "Construction Issue".
// Store: design:rock-drawings:<no> = [ { id, familyId, revision, title, name, url,
...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    const readMap = await rdGetReadMap(no, acc.user.id)
    const unread = rdUnread(docs, readMap)
  ...
  if (body.action === 'mark-read') {
    ...
    if (d) await rdMarkRead(no, acc.user.id, d)
  ...
  if (body.action === 'comment') {
    ...
    const i = idx(body.id)
    if (i < 0) return res.status(404).json({ error: 'Drawing not found' })
    ...
    try { await rdMarkRead(no, acc.user.id, docs[i]) } catch {}
    try { await rdRecordPendingComment(no) } catch {}
  ...
  if (body.action === 'approve') {
    const i = idx(body.id)
    if (i < 0) return res.status(404).json({ error: 'Drawing not found' })
    if (!acc.external) return res.status(403).json({ error: 'Only the customer can approve a drawing.' })
    if (docs[i].approverId && docs[i].approverId !== acc.user.id) return res.status(403).json({ error: 'Only the assigned approver can approve this drawing.' })
  ...
  if (body.action === 'add') {
    ...
    const doc = {
      id: rid('rd'), familyId: rid('fam'), revision: 'A',
      title: (body.title || f.name || 'Drawing').trim(),
      name: f.name || 'Drawing', url: f.url, contentType: f.contentType || '', size: f.size || 0,
    ...
    try { await rdRecordPendingDoc(no) } catch {}
  ...
  if (body.action === 'add-revision') {
    ...
    if (!base) return res.status(404).json({ error: 'Drawing not found' })
    ...
    const doc = {
      id: rid('rd'), familyId: fam, revision: nextRev,
      title: base.title,
