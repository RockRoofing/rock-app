...
const HKEY = (no) => `design:leak-test-certs:${no}`
...
export default async function handler(req, res) {
  ...
  // Only allow URLs that belong to this project's warranties.
  const data = (await get(HKEY(projectNo))) || { files: [] }
  const allowed = new Map()
  for (const d of (data.files || [])) if (d.url) allowed.set(d.url, d.name)
