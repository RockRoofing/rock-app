  ...
  for (const no of nos) {
    ...
    if (recipients.length) {
      ...
      for (const r of recipients) {
        const html = techSubDigestHtml({ name: r.name, projectName, projectNo: no, comments: pend.comments })
        const out = await sendMail(r.email, `New comments on Tech Subs for ${projectName || no}`, html)
