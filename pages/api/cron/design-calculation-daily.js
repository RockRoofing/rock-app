  ...
  for (const no of nos) {
    ...
    if (recipients.length) {
      ...
      for (const r of recipients) {
        const html = calculationDigestHtml({ name: r.name, projectName, projectNo: no, comments: pend.comments, docs: pend.docs || 0 })
        const out = await sendMail(r.email, `New updates on Calculations for ${projectName || no}`, html)
...
export default forEachTenant('design-calculation-daily', handler)
