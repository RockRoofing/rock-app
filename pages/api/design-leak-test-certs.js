...
// Leak Test Certs for a project: a flat list of uploaded documents (no sections).
// Store: design:leak-test-certs:<no> = { files: [ { id, name, url, contentType, size,
//                                   uploadedBy, uploadedAt } ] }
const WKEY = (no) => `design:leak-test-certs:${no}`
...
export default async function handler(req, res) {
  ...
  try {
    if (body.action === 'add-file') {
      ...
      data.files.push({ id: rid('doc'), name: f.name || 'Leak Test Cert', url: f.url, contentType: f.contentType || '', size: f.size || 0, uploadedBy: acc.user.name || 'User', uploadedAt: Date.now() })
