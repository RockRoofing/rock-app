...
export default function RockDrawingsPage() {
  ...
  // Status counts (current items): every item is EITHER Construction Issue OR still for
  // review/comment - so the two always sum to the total.
      ...
      <div style={{ background: '#fff', borderRadius: 14, padding: 18, width: '96vw', height: '96vh', maxWidth: '96vw', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
          ...
          {dwg ? (
            ...
            <DrawingMarkup key={viewUrl(doc)} imageUrl={viewUrl(doc)} contentType={doc.contentType} initial={doc.markup} canEdit onSave={(m) => onMarkup(doc.id, m)} fileName={doc.name} docLabel="drawing" />
          ) : (
