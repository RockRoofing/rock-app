...
export default function CalculationsPage() {
  ...
  async function load() {
    ...
    try {
      const d = await fetch(`/api/design-calculations?no=${encodeURIComponent(projectNo)}`).then(r => r.json())
  ...
  function openDoc(id) {
    ...
    if (unread.includes(id)) {
      ...
      fetch('/api/design-calculations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectNo, action: 'mark-read', id }) }).catch(() => {})
    }
  }
  ...
  async function post(payload) {
    const r = await fetch('/api/design-calculations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectNo, ...payload }) })
  ...
  async function approve(id) { if (!confirm('Approve this calculation?')) return; await post({ action: 'approve', id }) }
  ...
  async function downloadZip(urls, zipName) {
    ...
    try {
      const r = await fetch('/api/design-calculations-zip', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectNo, urls, zipName }) })
  ...
  async function approveSelected() {
    ...
    if (!confirm(`Approve ${approvableSelectedCount} selected calculation${approvableSelectedCount === 1 ? '' : 's'}?`)) return
  ...
  return (
    <>
      ...
      <DesignNav active="calculations" projectNo={projectNo} projectName={auth.project && auth.project.name} isInternal={auth.isInternal} />
      ...
      <div style={{ width: '100%', margin: 0, padding: '22px 24px 60px', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
          <div>
            <h1 style={{ margin: '0 0 2px', color: INK, fontSize: 24 }}>Calculations</h1>
        ...
        {currentDocs.length > 0 && (reviewCount > 0
          ...
          : <div style={{ background: '#dbeafe', border: '1px solid #bfdbfe', color: '#1d4ed8', borderRadius: 8, padding: '11px 14px', fontSize: 15, fontWeight: 800, letterSpacing: 0.5, marginBottom: 12 }}>STATUS: {reviewCount} FOR REVIEW / COMMENT&nbsp;&nbsp;&middot;&nbsp;&nbsp;{constructionCount} CONSTRUCTION ISSUE</div>)}
      ...
      {openDocObj && <CalcViewer doc={openDocObj} people={people} personName={personName} onClose={() => setOpenId(null)} onComment={addComment} onMarkup={saveMarkup}
...
function CalcViewer({ doc, people, personName, onClose, onComment, onMarkup, canApprove, onApprove, approverName }) {
  ...
  return (
    <div onClick={onClose} style={modalWrap}>
      <div onClick={e => e.stopPropagation()} style={{ ...modalCard, width: 480 }}>
        ...
        <p style={{ fontSize: 13, color: '#8a857c', marginTop: 0 }}>Everyone on this project will be told calculations have been uploaded. Tick anyone who needs to <strong>approve</strong> - they'll be asked to review and approve. Everyone else is just notified.</p>
