...
export default function LeakTestCertsPage() {
  ...
  async function load() {
    ...
    try {
      const d = await fetch(`/api/design-leak-test-certs?no=${encodeURIComponent(projectNo)}`).then(r => r.json())
  ...
  async function post(payload) {
    const r = await fetch('/api/design-leak-test-certs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectNo, ...payload }) })
  ...
  async function deleteFile(fileId) {
    if (!confirm('Delete this leak test cert?')) return
  ...
  async function downloadZip(urls, zipName) {
    ...
    try {
      const r = await fetch('/api/design-leak-test-certs-zip', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectNo, urls, zipName }) })
  ...
  return (
    <>
      <Head><title>Leak Test Certs - Design</title></Head>
      ...
      <div style={{ width: '100%', margin: 0, padding: '22px 24px 60px', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
          <div>
            <h1 style={{ margin: '0 0 2px', color: INK, fontSize: 24 }}>Leak Test Certs</h1>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {canEdit && <button onClick={() => inputRef.current && inputRef.current.click()} disabled={uploading} style={{ ...btnPrimary, opacity: uploading ? 0.6 : 1 }}>{uploading ? 'Uploading...' : '+ Upload Leak Test Certs'}</button>}
          </div>
        </div>
        ...
        {loading ? <div style={{ color: '#999', padding: 20 }}>Loading...</div>
          : files.length === 0 ? (
            <div style={{ background: '#fff', border: '1px solid #ececec', borderRadius: 12, padding: 40, textAlign: 'center', color: '#aaa' }}>
              No Leak Test Certs Yet. {canEdit ? 'Use "+ Upload Leak Test Certs" to add documents.' : 'Nothing has been added yet.'}
            </div>
          ) : (
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 14, padding: 16, width: '94vw', height: '94vh', maxWidth: '94vw', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', minHeight: 0, background: '#f4f4f4', borderRadius: 8 }}>
          {isImg(file)
            ...
            : viewable
              ? <div style={{ padding: 10 }}><DrawingMarkup imageUrl={file.url} contentType={file.contentType} initial={null} canEdit={false} onSave={() => {}} fileName={file.name} docLabel="leak test cert" /></div>
