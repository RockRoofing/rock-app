...
// Shared page: Warranties, O&Ms, Calculations, Tech Sub (view), Leak Test Certs.
// Project comes from the route (/design/[project]/<slug>).
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 1600, margin: '0 auto', padding: '22px 28px 60px' }}>
        ...
        {err && <div style={{ background: '#fef2f2', border: '1px solid #fecaca', color: '#b91c1c', borderRadius: 8, padding: '9px 12px', fontSize: 13, margin: '12px 0' }}>{err}</div>}

        {techSub && currentTechSub && (
          <div style={{ margin: '14px 0 18px', border: '1px solid #ece9f5', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ background: '#faf9fd', padding: '10px 14px', fontSize: 13.5, fontWeight: 700, color: INK, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <a href={`/api/download?url=${encodeURIComponent(currentTechSub.url)}&name=${encodeURIComponent(currentTechSub.name)}&inline=1`} target="_blank" rel="noreferrer" style={{ color: PURPLE, fontSize: 13, textDecoration: 'none', fontWeight: 600 }}>Open in new tab</a>
            </div>
        ...
        {loading ? <div style={{ color: '#999', padding: 20 }}>Loading...</div> : (
          <div style={{ marginTop: 8, background: '#fff', border: '1px solid #ececec', borderRadius: 12, overflow: 'auto' }}>
